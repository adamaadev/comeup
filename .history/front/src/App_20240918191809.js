...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (id) { // Ensure id is available before checking subscription
      axios.post('http://localhost:4000/checksubs', { id }).then(res => {
        console.log(res);

        if (!res.data.status) {
          navigate('/abonnement'); // Use the navigate function instead of Navigate component
        }
      });
    }
  }, [id, navigate]); // Add dependencies to useEffect
