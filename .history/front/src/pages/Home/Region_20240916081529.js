...
export default function Region({ symbol }) {
  ...
  return (
    <div className="chart-container" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '5rem' }}>
      {hasData ? (
        <>
          {/* Geographic Revenue Chart */}
          <div style={{ width: '45%' }}>
            <h3 style={{ textAlign: 'center' }}>Segmentation Géographique</h3>
          ...
          <div style={{ width: '45%' }}>
            <h3 style={{ textAlign: 'center' }}>Segmentation Produit</h3>
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
                  data={productData}
