...
export default function Vente({ symbol }) {
  ...
  const renderCustomLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, percent, index }, data) => {
    ...
    const radius = outerRadius + 10; // Small offset from the pie slice
    const x = cx + radius * Math.cos(-midAngle * RADIAN); // Label x position
    const y = cy + radius * Math.sin(-midAngle * RADIAN); // Label y position
  
    const lineEndX = cx + (outerRadius + 25) * Math.cos(-midAngle * RADIAN); // End of the line x
    const lineEndY = cy + (outerRadius + 25) * Math.sin(-midAngle * RADIAN); // End of the line y
  
    return (
      <>
        {/* Line segment connecting the pie slice to the label */}
        <line
