...
export default function Screener() {
  ...
  useEffect(() => {
    axios.get('http://localhost:4000/screener')
      .then(res => {
        // Appliquer le filtre pour les entreprises basées aux États-Unis
        const filteredData = res.data.map(info => {
          if (info.pays === 'United States' && (info.symbol.includes('.') || info.symbol.includes('-'))) {
            // Exclure celles qui ont un symbole avec un point ou un tiret
            return null;
          }
          return info;
        }).filter(info => info !== null); // Supprimer les résultats "null"
  
        const sortedData = filteredData.sort((a, b) => b.Capitalisation - a.Capitalisation); // Tri décroissant
        setInfos(sortedData);
