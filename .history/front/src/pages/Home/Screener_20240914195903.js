...
export default function Screener() {
  ...
  useEffect(() => {
    axios.get('http://localhost:4000/screener')
      .then(res => {
        // Exclure les entreprises avec un point dans le symbole au chargement initial
  ...
  useEffect(() => {
    const applyFilters = () => {
      // Si aucune recherche n'est effectuée, exclure les symboles avec un point
      if (query === '') {
        return infos;
      }
      ...
      // Inclure les entreprises avec un point dans le symbole si elles correspondent à la recherche
      const allResults = res.data.filter(info => 
        info.symbol.toLowerCase().includes(query.toLowerCase()) || 
        info.Nom.toLowerCase().includes(query.toLowerCase())
      );
