...
export default function Vente({ symbol }) {
  ...
  return (
    <div className="chart-wrapper" style={{ display: 'flex', width: '100%', height:'150%', alignItems: 'center', gap: '3rem' }}>
      {productData.length > 0 ? (
        <div style={{ width: '65%', position: 'relative', background:'red'}}> {/* Reduced width */}
          <h3>Segmentation Produit</h3>
          <ResponsiveContainer width="100%" height={400}> {/* Reduced height */}
            <PieChart>
              <Pie
                data={productData}
                cx="50%"
                ...
                innerRadius={80} // Reduced innerRadius
                fill="#8884d8"
                dataKey="value"
                label={(props) => renderCustomLabel(props, productData)}
              >
                {productData.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={predefinedColors[index % predefinedColors.length-5]} />
                ))}
              </Pie>
              <Tooltip />
            </PieChart>
          ...
          {logo && (
            <div
              style={{
                ...
                width: '70px', // Reduced width of logo
                height: '70px', // Reduced height of logo
                overflow: 'hidden',
                marginTop: '10px'
              }}
            >
