  ...
  useEffect(() => {
    axios.get('http://localhost:4000/screener')
      .then(res => {
        ...
        // Filtrer les entreprises qui n'ont pas de point ou de tiret dans leur symbole au démarrage
        const filteredInitialData = sortedData.filter(info => !info.symbol.includes('.') && !info.symbol.includes('-'));
        setInfos(sortedData); // Toutes les données
        setFilteredInfos(filteredInitialData); // Données filtrées au démarrage
      });
  }, []);
