...
export default function Vente({ symbol }) {
  ...
  const renderCustomLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, percent, index }, data) => {
    ...
    const radius = outerRadius + 40; // Increase the distance of the label from the pie slice
    const x = cx + radius * Math.cos(-midAngle * RADIAN); // New label x position
    const y = cy + radius * Math.sin(-midAngle * RADIAN); // New label y position
  
    const lineStartX = cx + outerRadius * Math.cos(-midAngle * RADIAN); // Start of the line at the outer edge of the pie
    const lineStartY = cy + outerRadius * Math.sin(-midAngle * RADIAN); // Start of the line at the outer edge of the pie
    const lineEndX = cx + (outerRadius + 50) * Math.cos(-midAngle * RADIAN); // End of the line with increased length
    const lineEndY = cy + (outerRadius + 50) * Math.sin(-midAngle * RADIAN); // End of the line with increased length
  
    return (
      <>
        {/* Longer line from pie slice to label */}
        <line
          x1={lineStartX}
          y1={lineStartY}
          x2={lineEndX}
  ...
  return (
    <div className="chart-wrapper" style={{ width: '100%', alignItems: 'center', justifyContent: 'space-between', gap: '3rem' }}>
