...
export default function Screener() {
  ...
  useEffect(() => {
    axios.get('http://localhost:4000/screener')
      .then(res => {
        setInfos(res.data);
        setFilteredInfos(res.data); // Initialement, les infos filtrées sont les mêmes que les infos triées
      });
  }, []);
    ...
    <section className="container mt-4">
      <div className="row align-items-center mb-4">
        <div className="col-md-6">
          <h2 className="mb-0">Screener Quality {filteredInfos.length}</h2>
        </div>
