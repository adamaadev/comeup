...
export default function Region({ symbol }) {
  ...
  {hasData ? (
    <>
      ...
      <div style={{ width: '50%', position: 'relative' }}> {/* Ajuster la largeur du conteneur */}
        <h3>Segmentation Géographique</h3>
        <ResponsiveContainer width="100%" height={250}> {/* Réduire la hauteur */}
          <PieChart>
            <Pie
              data={geoData}
              cx="50%"
              cy="50%"
              innerRadius={60} {/* Réduire innerRadius */}
              outerRadius={100} {/* Réduire outerRadius */}
              fill="#8884d8"
              dataKey="value"
        ...
        {logo && (
          <div
            style={{
              ...
              width: '80px', {/* Ajuster la taille du logo */}
              height: '80px',
              overflow: 'hidden',
              marginTop: '10px'
            }}
