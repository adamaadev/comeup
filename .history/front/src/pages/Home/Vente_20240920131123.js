...
export default function Vente({ symbol }) {
  ...
  const renderLabelLine = (props) => {
    ...
    const RADIAN = Math.PI / 180;
    const radius = outerRadius + 80; // Ajustez cette valeur pour augmenter la longueur des lignes
    const x = cx + radius * Math.cos(-midAngle * RADIAN);
    const y = cy + radius * Math.sin(-midAngle * RADIAN);
  
    // Zone d'exclusion pour éviter le logo (50px autour du centre, ajustez si nécessaire)
    const exclusionRadius = 50;
  
    // Si le point (x, y) est trop proche du centre, ne pas dessiner la ligne
    if (Math.sqrt((x - cx) ** 2 + (y - cy) ** 2) < exclusionRadius) {
      return null; // Ne pas dessiner la ligne dans la zone d'exclusion
    }
  ...
  labelLine={renderLabelLine}  // Ajoutez cette ligne pour utiliser la fonction de ligne personnalisée
>
