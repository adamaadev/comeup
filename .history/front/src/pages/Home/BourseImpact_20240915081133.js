...
export default function BourseImpact() {
  ...
  useEffect(() => {
    const savedColumns = JSON.parse(localStorage.getItem('columns1'));
    console.log('Colonnes lues depuis localStorage:', savedColumns);
    if (savedColumns) {
  ...
  const handleDragEnd = (result) => {
    ...
    localStorage.setItem('columns1', JSON.stringify(newColumns));
                    ...
                    {columns.map((col, idx) => (
                      <td
                        ...
                        style={{
                          ...
                          paddingRight: col === 'Capitalisation' ? '50px' : '0',
                        }}
                      >
                        {col === 'Nom' ? (
                          <div className="d-flex align-items-center">
