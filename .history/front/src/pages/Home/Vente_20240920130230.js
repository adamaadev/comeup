...
export default function Vente({ symbol }) {
  ...
  // Custom function for label rendering, pointing the labels towards the legend area
  ...
  const renderLabelLine = (props) => {
    ...
    const RADIAN = Math.PI / 180;
    const radius = outerRadius + 60; // Control the length to point towards the legend
    const x = cx + radius * Math.cos(-midAngle * RADIAN);
    const y = cy + radius * Math.sin(-midAngle * RADIAN);
