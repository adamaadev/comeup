  ...
  const handleQueryChange = (event) => {
    ...
    if (newQuery === '') {
      ...
      const filtered = infos.filter(info => {
        const nom = info.Nom ? info.Nom.toLowerCase() : '';
        const symbol = info.symbol ? info.symbol.toLowerCase() : '';
  
        // Vérifier si le symbole est une correspondance exacte
        const isExactMatch = symbol === newQuery;
  
        // Vérifier si le symbole ou le nom a au moins 2 lettres communes avec la requête
        const hasCommonLetters = (str) => {
          let commonCount = 0;
          for (let char of newQuery) {
            if (str.includes(char)) {
              commonCount++;
            }
          }
          return commonCount <= 2; // Limite à 2 lettres communes
        };
  
        // Filtrer les résultats : garder les symboles exacts ou ceux ayant 2 lettres ou moins en commun
        return isExactMatch || hasCommonLetters(symbol) || hasCommonLetters(nom);
      });
