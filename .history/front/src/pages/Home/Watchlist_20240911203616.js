...
export default function Watchlist() {
  ...
  const handleQueryChange = (event) => {
    ...
    if (newQuery === '') {
      ...
      // Filtrer les résultats par symbole exact
      let filtered = infos.filter(info =>
        info.symbol && info.symbol.toLowerCase() === newQuery
      );
  
      if (filtered.length === 0) {
        // Si aucun résultat exact, filtrer par symboles avec plus de 2 lettres communes
        filtered = infos.filter(info =>
          countCommonLetters(info.symbol, newQuery) > 2
        );
  
        // Assurer que seul un résultat est affiché
        if (filtered.length > 0) {
          filtered = [filtered[0]]; // Conserver uniquement le premier résultat
        }
      }
