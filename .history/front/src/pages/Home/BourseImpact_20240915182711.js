...
export default function BourseImpact() {
  ...
  useEffect(() => {
    axios.post('http://localhost:4000/watchlist', { type : 'admin'})
      .then(res => {
  ...
  <div className="col-12 d-flex justify-content-between align-items-center">
    ...
    <input
      type="text"
      className="form-control ms-3 w-100 w-md-55 w-lg-50"
  ...
  <tbody>
    {currentItems.map((info, index) => (
      ...
      >
        ...
        <td className="text-center" style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          {info.Pea === 1 ? <FaCheck color="green" /> : <FaTimes color="red" />}
        </td>
        <td className="text-center" style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
          {info.Dividende === 1 ? <FaCheck color="green" /> : <FaTimes color="red" />}
        </td>
