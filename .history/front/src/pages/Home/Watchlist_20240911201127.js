...
export default function Watchlist() {
  ...
  const handleQueryChange = (event) => {
    ...
    if (newQuery === '') {
      setFilteredInfos(infos); // Réinitialiser les résultats filtrés
    } else {
      const filtered = infos.filter(info =>
        info.Nom.toLowerCase().includes(newQuery.toLowerCase()) || 
        info.symbol.toLowerCase().includes(newQuery.toLowerCase())
      );
