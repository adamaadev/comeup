...
export default function Prix({ symbol }) {
  ...
  return (
    <div style={{ backgroundColor: '#ffffff', color: '#000', padding: '20px', borderRadius: '10px' }}>
      <h2 style={priceStyle}>
        Prix actuel: {currentPrice} USD {currentPrice > previousPrice ? `(+${(currentPrice - previousPrice).toFixed(2)} / +${priceChangePercentage}%)` : `(${(currentPrice - previousPrice).toFixed(2)} / ${priceChangePercentage}%)`}
      </h2>
