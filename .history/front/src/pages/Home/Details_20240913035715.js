...
export default function Details() {
  ...
  const handleTabChange = (tab) => {
    setActiveTab(tab);
    localStorage.setItem('activeTab', tab); // Enregistrer l'onglet actif dans localStorage
  };
    ...
    <div className="container mt-4">
      <div className="d-flex flex-column flex-md-row justify-content-between align-items-start mb-4">
        <div className="d-flex align-items-center mb-3 mb-md-0">
          ...
          />
          <div>
            <h2 className="company-name mb-1">{infos.Nom}</h2>
            <p className="stock-price mb-0">{infos.symbol}</p>
          </div>
        </div>
        <div>
          {exist ? (
      ...
      <ul className="nav nav-tabs">
        <li className="nav-item">
          <button className={`nav-link ${activeTab === 'Informations' ? 'active' : ''}`} onClick={() => handleTabChange('Informations')}>
            Informations
          </button>
        </li>
        <li className="nav-item">
          <button className={`nav-link ${activeTab === 'quant' ? 'active' : ''}`} onClick={() => handleTabChange('quant')}>
            Analyse Qualitative
          </button>
        </li>
        <li className="nav-item">
          <button className={`nav-link ${activeTab === 'qual' ? 'active' : ''}`} onClick={() => handleTabChange('qual')}>
      ...
      <div className="tab-content mt-4">
        {activeTab === 'Informations' && (
          <div className="d-flex flex-column flex-md-row">
            ...
            <div className="info-section flex-fill">
