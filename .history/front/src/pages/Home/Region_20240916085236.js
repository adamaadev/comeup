...
export default function Region({ symbol }) {
  ...
  return (
    <div
      ...
      style={{
        display: 'flex',
        flexDirection: 'column', // Ensure vertical layout on smaller screens
        width: '100%',
        alignItems: 'center',
    ...
    >
      {hasData ? (
        <>
          {/* Business Segment Revenue Chart */}
          <div style={{ width: '100%', maxWidth: '600px', position: 'relative' }}>
            <h3>Segmentation par Secteur</h3>
            <ResponsiveContainer width="100%" height={400}>
              <PieChart>
                <Pie
                  data={geoData} {/* Replace businessData with geoData */}
                  cx="50%"
                  cy="50%"
                  innerRadius={100}
                  outerRadius={140}
                  fill="#8884d8"
                  dataKey="value"
                  label={(props) => renderCustomLabel(props, geoData)} {/* Replace businessData with geoData */}
                >
