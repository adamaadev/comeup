...
export default function Watchlist() {
  ...
  const handleQueryChange = (event) => {
    const newQuery = event.target.value.toLowerCase(); // Saisie en minuscule pour comparaison
    setQuery(newQuery);
    ...
    if (newQuery === '') {
      ...
      const filtered = infos.filter(info =>
        info.Nom.toLowerCase().includes(newQuery) || 
        info.symbol.toLowerCase().includes(newQuery)
      );
