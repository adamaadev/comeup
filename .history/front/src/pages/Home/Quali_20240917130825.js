...
export default function Quali({ symbol }) {
  ...
  const renderRatioRow = (label, value, isSpecialFormat = false) => {
    ...
    const formattedValue = isSpecialFormat ? formatNumber(numValue) : `${numValue}%`;
    return (
      <div className="d-flex align-items-center mb-2">
        <span className="me-3">{label} :</span>
        <div className="progress" style={{ width: '20%', marginRight: '10px' }}>
          <div
            className={`progress-bar ${getBarColor(numValue)}`}
            role="progressbar"
            style={{ width: `${Math.min(numValue, 100)}%`, textAlign: 'center' }}
