...
export default function Screener() {
  ...
  useEffect(() => {
    axios.get('http://localhost:4000/screener')
      .then(res => {
        const sortedData = res.data.sort((a, b) => b.Capitalisation - a.Capitalisation); // Tri décroissant par capitalisation boursière
        setInfos(sortedData);
  ...
  useEffect(() => {
    const applyFilters = () => {
      ...
      // Appliquer les filtres définis
      filtered = filtered.filter(info => {
        return filters.every(({ filterType, selectedOption, min, max }) => {
          if (!filterType) return true; // Aucun filtre pour ce champ
  
          // Filtrage par PEA éligible
          ...
          if (['buyback_yield', 'fcf_1_year', 'fcf_5_years', 'fcf_10_years', 'fcf_margin_one_year', 'fcf_margin_five_year', 'roce', 'roce_5_year_avg', 'croissance_resultat_net_1_an', 'croissance_resultat_net_5_ans', 'croissance_CA_1_an', 'croissance_CA_5_ans', 'croissance_CA_10_ans', 'croissance_annualisee', 'croissance_moyenne', 'debt_equity', 'ratio_payout', 'performance', 'nbreannee'].includes(filterType)) {
            ...
            if (isNaN(value)) return true; // Si la valeur n'est pas un nombre, on l'ignore
            if (min && value < parseFloat(min)) return false; // Vérifier la valeur minimale
            if (max && value > parseFloat(max)) return false; // Vérifier la valeur maximale
            return true;
          }
  
          // Comparaison par option sélectionnée
          if (selectedOption) {
      ...
      filtered.forEach(info => {
        ...
        if (info.symbol.toLowerCase().includes(key) || info.Nom.toLowerCase().includes(key)) {
          if (!uniqueResults.has(info.Nom)) {
            ...
            // Comparez les suffixes ou symboles pour sélectionner le plus pertinent
