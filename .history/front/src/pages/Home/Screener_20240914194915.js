  ...
  useEffect(() => {
    axios.get('http://localhost:4000/screener')
      .then(res => {
        ...
        // Filtrer les entreprises qui ont 3 lettres ou moins avant le point dans le symbole
        const filteredData = sortedData.filter(info => {
          const parts = info.symbol.split('.');
          return parts[0].length <= 1;  // Garde uniquement les entreprises avec 3 lettres ou moins avant le '.'
        });
  
        setInfos(filteredData);
        setFilteredInfos(filteredData); // Initialement, les infos filtrées sont les mêmes que les infos triées
      });
  }, []);
      ...
      const filtered = infos.filter(info => {
        ...
        const matchesFilters = filters.every(({ filterType, selectedOption, min, max }) => {
          ...
          if (['buyback_yield', 'fcf_1_year', 'fcf_5_years', 'fcf_10_years', 'fcf_margin_one_year', 'fcf_margin_five_year', 'roce', 'roce_5_year_avg', 'croissance_resultat_net_1_an', 'croissance_resultat_net_5_ans', 'croissance_CA_1_an', 'croissance_CA_5_ans', 'croissance_CA_10_ans', 'croissance_annualisee', 'croissance_moyenne', 'debt_equity', 'ratio_payout', 'performance', 'nbreannee'].includes(filterType)) {
            const value = parseFloat(info[filterType]); // Convertir la valeur en nombre
            
            // Gérer les NaN (valeurs manquantes) en les excluant du filtrage
            if (isNaN(value)) return true;
            
            // Vérification des conditions min et max
            const minValue = min ? parseFloat(min) : null;
            const maxValue = max ? parseFloat(max) : null;
  
            if (minValue !== null && value < minValue) return false; // Exclure si inférieur au minimum
            if (maxValue !== null && value > maxValue) return false; // Exclure si supérieur au maximum
