...
export default function Screener() {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then(res => {
        ...
        const filteredData = res.data.filter(company =>
          !company.symbol.includes('.') &&
          !company.symbol.includes('-') &&
          company.pays.toLowerCase() !== 'etats unis'
        );
        
        setCompanies(filteredData);
        setFilteredCompanies(filteredData); // Initially, show filtered companies
      });
  }, []);
  ...
  return (
    <div>
      <h2>Company Screener {companies.length}</h2>
