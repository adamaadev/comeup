...
export default function Quati({ symbol }) {
  ...
  const calculateScore = (performance, croissance_CA_1_an , croissance_CA_5_ans , croissance_CA_10_ans , buyback_yield, croissance_annualisee, croissance_moyenne, croissance_resultat_net_1_an, croissance_resultat_net_5_ans, debt_equity, fcf_1_year, fcf_5_years, fcf_10_years, fcf_margin_five_year, fcf_margin_one_year, nbreannee, piotroski_score, rachat_net_moyen, ratio_capex_revenu_net, ratio_payout, roce, roce_5_year_avg) => {
    let newScore = 0;
    ...
    if (fcf_1_year > 9) {
      newScore += 1;
    } else if (fcf_1_year >= 5 && croissance_CA_10_ans <= 9) {
      newScore += 0.5;
    }

    if (fcf_5_years > 9) {
      newScore += 1;
    } else if (fcf_5_years >= 5 && fcf_5_years <= 9) {
      newScore += 0.5;
    }
