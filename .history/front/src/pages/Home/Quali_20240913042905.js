...
export default function Quali({ symbol }) {
  ...
  const calculateQuantiScore = (data) => {
    ...
    // Normaliser le score à une échelle de 20
    const maxScore = ratios.length; // Puisque chaque ratio peut donner au maximum 1 point
    const normalizedScore = (score / maxScore) * 20;

    setQuantiScore(normalizedScore);
  };
  ...
  return (
    <div>
      ...
      <h2>Score Quantitatif: {quantiScore.toFixed(2)}</h2>
    </div>
  );
}
