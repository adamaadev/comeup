...
export default function Quali({ symbol }) {
  ...
  const formatNumber = (value) => {
    if (typeof value !== 'number' || isNaN(value)) {
      return 'N/A'; // Return a placeholder if the value is not a number
    }
  ...
  const renderRatioRow = (label, value, isSpecialFormat = false) => {
    const formattedValue = isSpecialFormat ? formatNumber(getNumber(value)) : `${getNumber(value)}%`;
    return (
      <div className="d-flex align-items-center mb-2">
        <span className="me-3">{label} :</span>
        <div className="progress" style={{ width: '20%', marginRight: '10px' }}>
          <div
            className={`progress-bar ${getBarColor(getNumber(value))}`}
            role="progressbar"
            style={{ width: `${Math.min(getNumber(value), 100)}%`, textAlign: 'center' }}
            aria-valuenow={getNumber(value)}
            aria-valuemin="0"
            aria-valuemax="100"
          >
            {formattedValue}
        ...
        <div className="card-body">
          <div className="row">
            ...
            <div className="col-md-6 col-lg-4 mb-4">
              ...
              {renderRatioRow('Rachat net d\'actions (Moy 5A)', infos.rachat_net_moyen, true)} {/* Formatage spécial pour rachat net */}
            </div>
          </div>
        </div>
      </div>
