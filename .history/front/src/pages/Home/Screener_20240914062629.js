    ...
    const applyFilters = () => {
      ...
      const filtered = infos.filter(info => {
        ...
        const matchesFilters = filters.every(({ filterType, selectedOption, min, max }) => {
          ...
          if (isNaN(value)) return true; // Si la valeur n'est pas un nombre, ne pas filtrer
  
          // Appliquer les filtres min et max seulement si ces champs sont définis
          if (min && value < parseFloat(min)) return false;
          if (max && value > parseFloat(max)) return false;
  
          if (!selectedOption) return true; // Pas d'option spécifique sélectionnée
