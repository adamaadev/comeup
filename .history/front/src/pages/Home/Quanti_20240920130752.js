  ...
  <div className="card">
    ...
    <div className="card-body">
      <div className="row">
        ...
        <div className="col-md-6 col-lg-4 mb-4">
          <h5>Profits</h5>
          {['Résultat Net 1 an', 'Résultat Net 5 ans', 'CAPEX / Résultat Net (Moy 5A)'].map((label, idx) => (
            <div key={idx} style={{ display: 'inline-flex', alignItems: 'center' }}>
              <p style={{ marginBottom: '0', marginRight: '10px' }}>{label} :</p>
              <div className="progress" style={{ height: '13px', borderRadius: '5px', width: '100px', position: 'relative', flexGrow: 1 }}>
                <div
                  className={`progress-bar ${getProgressColor(infos[`croissance_resultat_net_${idx + 1}_an`])}`}
                  role="progressbar"
                  style={{ width: `${infos[`croissance_resultat_net_${idx + 1}_an`]}%` }}
                  aria-valuenow={infos[`croissance_resultat_net_${idx + 1}_an`]}
                  aria-valuemin="0"
                  aria-valuemax="100"
                >
                  <span style={{ position: 'absolute', left: '50%', transform: 'translateX(-50%)', color: 'Black' }}>
                    {infos[`croissance_resultat_net_${idx + 1}_an`]}%
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="col-md-6 col-lg-4 mb-4">
          <h5>Rentabilité</h5>
          {['ROCE 1 an', 'ROCE 5 ans', 'Free Cash Flow Margin 1 an', 'Free Cash Flow Margin 5 ans'].map((label, idx) => (
            <div key={idx} style={{ display: 'inline-flex', alignItems: 'center' }}>
              <p style={{ marginBottom: '0', marginRight: '10px' }}>{label} :</p>
              <div className="progress" style={{ height: '13px', borderRadius: '5px', width: '100px', position: 'relative', flexGrow: 1 }}>
                <div
                  className={`progress-bar ${getProgressColor(infos[`roce_${idx + 1}_an`])}`}
                  role="progressbar"
                  style={{ width: `${infos[`roce_${idx + 1}_an`]}%` }}
                  aria-valuenow={infos[`roce_${idx + 1}_an`]}
                  aria-valuemin="0"
                  aria-valuemax="100"
                >
                  <span style={{ position: 'absolute', left: '50%', transform: 'translateX(-50%)', color: 'Black' }}>
                    {infos[`roce_${idx + 1}_an`]}%
                  </span>
                </div>
              </div>
            </div>
        ...
        {/* Other sections (Santé, Dividende, Autre) follow a similar pattern */}
      </div>
    </div>
  </div>
</div>
