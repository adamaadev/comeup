      ...
      {isSidebarOpen && (
        <aside className={`bg-light border-start p-3 position-fixed top-0 end-0 vh-100 sidebar ${isSidebarOpen ? 'open' : ''}`} style={{ minWidth: '250px', zIndex: '1000' }}>
          ...
          <nav className="d-flex flex-column mt-5">
            <NavLink to={'/watchlist'} className="sidebar-link mb-3">Watchlist</NavLink>
            <NavLink to={'/bareme'} className="sidebar-link mb-3">Bareme</NavLink>
            <NavLink to={'/screener'} className="sidebar-link mb-3">Screener</NavLink>
            <NavLink to={'/compte'} className="sidebar-link mb-3">Compte</NavLink>
            <a href="mailto:contact@example.com" className="sidebar-link mb-3">Nous contacter !</a>
            <Link onClick={handleLogout} className="sidebar-link mb-3">Se déconnecter</Link>
          </nav>
        </aside>
      )}
    </div>
