import React, { useState, useEffect } from 'react';
import axios from 'axios';

export default function Bareme() {
    const [financialData, setFinancialData] = useState([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        axios.get('http://localhost:4000/api/ratios')
            .then(response => {
                setFinancialData(response.data);
                setLoading(false);
            });
    }, []);

    if (loading) {
        return <div>Loading...</div>;
    }

    const renderRow = (notation) => {
        return (
            <tr>
                <th style={{ padding: '10px', border: '1px solid #ddd' }}>{notation}</th>
                <td>{getDataByNotation(notation, 'croissance_CA_1_an')}</td>
                <td>{getDataByNotation(notation, 'croissance_CA_5_ans')}</td>
                <td>{getDataByNotation(notation, 'croissance_CA_10_ans')}</td>
            </tr>
        );
    };

    const getDataByNotation = (notation, key) => {
        const data = financialData.find(item => item[key + '_notation'] === notation);
        return data ? data[key] : 'faux';
    };

    return (
        <div style={{ padding: '20px' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                <thead>
                    <tr>
                        <th rowSpan="2" style={{ border: '2px solid #ddd', padding: '10px' }}>Notation</th>
                        <th style={{ border: '2px solid #ddd', padding: '10px' }}>Croissance CA 1 an</th>
                        <th style={{ border: '2px solid #ddd', padding: '10px' }}>Croissance CA 5 ans</th>
                        <th style={{ border: '2px solid #ddd', padding: '10px' }}>Croissance CA 10 ans</th>
                    </tr>
                </thead>
                <tbody>
                    {renderRow('Excellente')}
                    {renderRow('Correct')}
                    {renderRow('Faible')}
                </tbody>
            </table>
        </div>
    );
}
