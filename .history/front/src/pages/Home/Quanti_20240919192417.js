...
export default function Quati({ symbol }) {
  ...
  const calculateScore = (
    ...
    if (debt_equity > 10) {
      newScore += 1;
    } else if (debt_equity >= 6 && debt_equity <= 10) {
      newScore += 0.5;
    }

    if (ratio_payout > 10) {
      newScore += 1;
    } else if (ratio_payout >= 6 && ratio_payout <= 10) {
      newScore += 0.5;
    }

    if (croissance_annualisee > 10) {
      newScore += 1;
    } else if (croissance_annualisee >= 6 && croissance_annualisee <= 10) {
      newScore += 0.5;
    }

    if (croissance_moyenne> 10) {
      newScore += 1;
    } else if (croissance_moyenne >= 6 && croissance_moyenne <= 10) {
      newScore += 0.5;
    }

    if (buyback_yield > 10) {
      newScore += 1;
    } else if (buyback_yield >= 6 && buyback_yield <= 10) {
      newScore += 0.5;
    }
    
    if (piotroski_score > 10) {
      newScore += 1;
    } else if (piotroski_score >= 6 && piotroski_score <= 10) {
      newScore += 0.5;
    }

    if (nbreannee > 10) {
      newScore += 1;
    } else if (nbreannee >= 6 && nbreannee <= 10) {
      newScore += 0.5;
    }

    if (rachat_net_moyen > 10) {
      newScore += 1;
    } else if (rachat_net_moyen >= 6 && rachat_net_moyen <= 10) {
      newScore += 0.5;
    }
