...
app.post('/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  ...
  console.log('Webhook payload:', req.body.toString()); // Convertir le buffer en chaîne
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    const plan = session.display_items[0]?.custom?.name || 'Unknown'; // Utiliser une valeur par défaut si nécessaire
    ...
    db.query(updateSubscriptionQuery, [plan, new Date(), endDate, 'active', email], (err, result) => {
      if (err) {
