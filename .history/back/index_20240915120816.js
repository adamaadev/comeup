...
app.get('/subscribe', async (req, res) => {
    ...
    switch (plan.toLowerCase()) {
        case 'starter':
            priceId = 'price_1PqhMcFTPbYwB9q16TEha35N'; // Assurez-vous que cet ID est correct
            break;

        case 'pro':
            priceId = 'price_1PqhMcFTPbYwB9q16TEha35N'; // Utilisez l'ID correct pour le plan Pro
            break;
...
app.post('/webhook', bodyParser.raw({ type: 'application/json' }), (req, res) => {
    ...
    const endpointSecret = 'whsec_ebb9f10c1497412cc0c1777ea70f40937ad7230672abdd49f8db96828557c8c2'; // Remplacez par votre secret de webhook
    ...
    switch (event.type) {
        case 'checkout.session.completed':
            ...
            break;
        // Ajouter d'autres événements à gérer ici
        default:
    }
...
app.use(express.raw({ type: 'application/json' })); // Pour le webhook Stripe
...
app.get('/subscribe', async (req, res) => {
  ...
  switch (plan.toLowerCase()) {
    case 'starter':
      priceId = 'price_1PqhMcFTPbYwB9q16TEha35N'; // Remplacez par le bon priceId
      break;

    case 'pro':
      priceId = 'price_1PqhMcFTPbYwB9q16TEha35N'; // Assurez-vous d'avoir un priceId différent pour 'pro'
      break;
  ...
  try {
    ...
    res.send(session.url); // Envoyez l'URL de redirection à l'application React
  } catch (error) {
