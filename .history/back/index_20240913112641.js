...
app.post('/subscribe', async (req, res) => {
  try {
    const { priceId } = req.body; // Vous devez envoyer l'ID du plan ou du prix que vous avez défini dans Stripe
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      mode: 'subscription',
      success_url: 'http://localhost:3000/success', // URL de redirection après le succès
      cancel_url: 'http://localhost:3000/cancel', // URL de redirection après l'annulation
    });

    // Répondre avec l'URL de la session
