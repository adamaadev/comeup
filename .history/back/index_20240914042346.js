...
app.post("/", CheckUser, (req, res) => {
  console.log(// Obtenir la date actuelle du serveur
    const date_debut = new Date().toISOString().slice(0, 19).replace('T', ' ');
    );
...
app.post('/webhook', bodyParser.raw({ type: 'application/json' }), async (req, res) => {
  const endpointSecret = 'whsec_ebb9f10c1497412cc0c1777ea70f40937ad7230672abdd49f8db96828557c8c2'; // Remplace avec ton endpoint secret Stripe
  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      console.log('Paiement effectué :', session);

      // Ici tu peux aussi mettre à jour la base de données ou faire d'autres actions nécessaires
    }
