...
app.post('/watchlist', (req, res) => {
  ...
  // Requête pour obtenir les symboles de la watchlist en ordre de position
  con.query("SELECT symbol FROM watchlist WHERE id_user = ? AND type_user = ? ORDER BY position ASC", [id, type], (err, result) => {
    if (err) {
...
app.post('/save-order', (req, res) => {
  ...
  const queries = order.map((item, index) => {
    return new Promise((resolve, reject) => {
      con.query(
        "UPDATE watchlist SET position = ? WHERE id_user = ? AND symbol = ?",
        [index, id, item.symbol],
        (err, result) => {
          if (err) {
            reject(err);
          } else {
  ...
  Promise.all(queries)
    .then(() => res.status(200).send({ message: 'Order saved successfully' }))
    .catch(err => res.status(500).send({ error: 'Error saving order', details: err }));
});
