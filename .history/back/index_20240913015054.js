...
app.post('/essaie', async (req, res) => {
  try {
    ...
    db.query('UPDATE utilisateurs SET essaie = ?, carte = ?, status = ?, date_debut = ?, date_fin = ? WHERE id = ?',
      ['true', 0, 'active', trialStartDate, trialEndDate, userId],
      (error) => {
        if (error) {
          console.error(error);
          return res.status(500).json({ error: 'Erreur lors de la mise à jour des informations de l\'utilisateur.' });
        }
