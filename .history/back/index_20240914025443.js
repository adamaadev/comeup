...
app.use(bodyparser.json()); // Middleware général
...
app.post('/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    const email = session.customer_email; // Assumer que l'email est dans la session
    const plan = session.display_items[0].custom.name; // Assumer que le nom du plan est dans les items
    ...
    const updateSubscriptionQuery = 'UPDATE utilisateurs SET plan = ?, date_debut = ?, date_fin = ?, status = ? WHERE email = ?';
    db.query(updateSubscriptionQuery, [plan, new Date(), endDate, 'active', "adamadieng289@gmail.com"], (err, result) => {
      if (err) {
