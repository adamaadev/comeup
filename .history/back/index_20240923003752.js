...
import { AddCompany, AddCompanyadmin, checkcompany, checkcompanyadmin, deletecompany, deletecompanyadmin, ListCompanies, Watchlist, WatchlistUser } from "./services/Watchlist.js";
...
app.post('/addcompanyadmin', AddCompanyadmin);
...
app.post('/checkcompanyadmin', checkcompanyadmin);
...
app.post('/deletecompanyadmin', deletecompanyadmin);
...
app.get('/getscore', (req, res) => {
  // SQL queries to count the number of companies in each score range
  const query = `
    SELECT 
      SUM(CASE WHEN quanti > 15 THEN 1 ELSE 0 END) AS count_above_15,
      SUM(CASE WHEN quanti BETWEEN 10 AND 15 THEN 1 ELSE 0 END) AS count_between_10_and_15,
      SUM(CASE WHEN quanti < 10 THEN 1 ELSE 0 END) AS count_below_10
    FROM screener;
  `;
  ...
  con.query(query, (err, results) => {
    ...
    console.log('Number of companies with score above 15:', counts.count_above_15);
    console.log('Number of companies with score between 10 and 15:', counts.count_between_10_and_15);
    console.log('Number of companies with score below 10:', counts.count_below_10);
...
app.get('/subscribe', async (req, res) => {
  ...
  try {
      const session = await stripe.checkout.sessions.create({
          ...
          cancel_url: `${process.env.BASE_URL}/abonnement`,
      });
...
app.post('/webhook', bodyParser.raw({ type: 'application/json' }), async (req, res) => {
  ...
  if (event.type === 'checkout.session.completed') {
      ...
      con.query(
          'UPDATE utilisateurs SET essaie = "true", status = "active", carte = ?, date_debut = NOW(), date_fin = DATE_ADD(NOW(), INTERVAL 7 DAY), produit_id = ? WHERE email = ?',
...
app.post('/checksubs',(req,res)=>{
  con.query('SELECT status FROM utilisateurs WHERE id = ?',[req.body.id],(err,result)=>{
