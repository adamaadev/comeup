...
con.query("SELECT symbol, pays, currency FROM screener WHERE pays = 'Indonesie' LIMIT 1", (err, result) => {
  ...
  if (result.length > 0) {
    ...
    if (pays === 'États-Unis') {
      con.query('UPDATE screener SET Capitalisation = ?',[capitalisation])
    } else {
