...
app.post('/essaie', async (req, res) => {
  try {
    ...
    // Calculer la date de début et la date de fin de l'essai
    const startDate = new Date();
    const endDate = new Date(startDate);
    endDate.setDate(startDate.getDate() + 7); // Essai gratuit de 7 jours

    // Mettre à jour la base de données avec les informations de l'essai gratuit
    connection.query(
      'UPDATE utilisateurs SET essaie = ?, date_debut = ?, date_fin = ? WHERE id = ?',
      ['true', startDate, endDate, userId],
      (error, results) => {
        if (error) {
