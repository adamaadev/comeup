...
function getNotation(value) {
  ...
  if (isNaN(score)) return 'N/A'; // Pour les valeurs non numériques
...
app.get('/api/ratios', (req, res) => {
  const query = `
    ...
    FROM screener WHERE symbol = 'AAPL';
  `;
  ...
  con.query(query, (error, results) => {
    ...
    // Ajouter la notation à chaque ratio
    const ratios = results.map(result => ({
      ...result,
      croissance_annualisee_notation: getNotation(result.croissance_annualisee),
      croissance_moyenne_notation: getNotation(result.croissance_moyenne),
      debt_equity_notation: getNotation(result.debt_equity),
      ratio_payout_notation: getNotation(result.ratio_payout),
      performance_notation: getNotation(result.performance),
      buyback_yield_notation: getNotation(result.buyback_yield),
      croissance_CA_1_an_notation: getNotation(result.croissance_CA_1_an),
      croissance_CA_5_ans_notation: getNotation(result.croissance_CA_5_ans),
      croissance_CA_10_ans_notation: getNotation(result.croissance_CA_10_ans),
      fcf_1_year_notation: getNotation(result.fcf_1_year),
      fcf_5_years_notation: getNotation(result.fcf_5_years),
      fcf_10_years_notation: getNotation(result.fcf_10_years),
      fcf_margin_one_year_notation: getNotation(result.fcf_margin_one_year),
      fcf_margin_five_year_notation: getNotation(result.fcf_margin_five_year),
      roce_notation: getNotation(result.roce),
      roce_5_year_avg_notation: getNotation(result.roce_5_year_avg),
      croissance_resultat_net_1_an_notation: getNotation(result.croissance_resultat_net_1_an),
      croissance_resultat_net_5_ans_notation: getNotation(result.croissance_resultat_net_5_ans),
      piotroski_score_notation: getNotation(result.piotroski_score),
      ratio_capex_revenu_net_notation: getNotation(result.ratio_capex_revenu_net),
      rachat_net_moyen_notation: getNotation(result.rachat_net_moyen),
      nbreannee_notation: getNotation(result.nbreannee),
      quanti_notation: getNotation(result.quanti)
    }));
