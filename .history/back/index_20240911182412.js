...
function Update() {
  ...
  con.query("SELECT symbol, pays, currency FROM screener WHERE modifier = 'non' LIMIT 1", (err, result) => {
    if (err) throw err; // Ajoutez une gestion d'erreur pour la requête SQL

    if (result.length > 0) {
      const symbol = result[0].symbol;
      const pays = result[0].pays;
        ...
        .then(res => {
          ...
          if (pays === 'États-Unis') {
            con.query('UPDATE screener SET Capitalisation = ? WHERE symbol = ?', [capitalisation, symbol], (err, result) => {
              if (result) {
                con.query('UPDATE screener SET modifier = "oui" WHERE symbol = ?', [symbol], (fail, success) => {
                  if (success) {
      ...
      // Mise à jour de toutes les entreprises avec modifier = "non"
      con.query('UPDATE screener SET modifier = "non"', (err, result) => {
        if (err) throw err;
        console.log("Mise à jour de toutes les entreprises avec modifier = 'non' réussie.");
      });
    }
  });
}
