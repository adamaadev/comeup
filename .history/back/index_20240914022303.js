...
app.post('/create-checkout-session', async (req, res) => {
  const { plan, email } = req.body; // Récupérer l'email dans la requête
  let priceId;

  // Associez les produits aux IDs de prix sur Stripe
  if (plan === 'FREE') {
  ...
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      mode: 'subscription',
      customer_email: email, // Ajoutez l'email ici
...
// Route pour gérer les événements de webhook Stripe
app.post('/webhook', async (req, res) => {
  const sig = req.headers['stripe-signature'];
  const endpointSecret = 'whsec_ebb9f10c1497412cc0c1777ea70f40937ad7230672abdd49f8db96828557c8c2'; // Remplacez par votre clé secrète de webhook
  ...
  if (event.type === 'invoice.payment_succeeded') {
    ...
    const subscriptionPlan = invoice.lines.data[0].price.nickname; // Assurez-vous que vous avez cette information
    console.log(customerEmail , subscriptionPlan);
    
    // Déterminer les dates de début et de fin en fonction du plan
    let dateDebut = new Date();
    let dateFin;
    switch (subscriptionPlan) {
      case 'Essai gratuit':
        dateFin = new Date();
        dateFin.setDate(dateDebut.getDate() + 7); // Une semaine
        break;
      case 'Starter':
        dateFin = new Date();
        dateFin.setMonth(dateDebut.getMonth() + 1); // Un mois
        break;
      case 'Premium':
        dateFin = new Date();
        dateFin.setFullYear(dateDebut.getFullYear() + 1); // Un an
        break;
      default:
    ...
    try {
      await db.query(
        'UPDATE utilisateurs SET status = ? plan = ?, date_debut = ?, date_fin = ? WHERE email = ?',
        ["active" , subscriptionPlan, dateDebut, dateFin, customerEmail]
      );
