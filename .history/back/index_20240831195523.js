...
app.post('/save-order', (req, res) => {
  const { id_user, type_user, order } = req.body;

  const queries = order.map((symbol, index) => {
    return new Promise((resolve, reject) => {
      con.query(
        "UPDATE watchlist SET position = ? WHERE id_user = ? AND type_user = ? AND symbol = ?",
        [index, id_user, type_user, symbol],
        (err, result) => {
          if (err) {
            reject(err);
          } else {
