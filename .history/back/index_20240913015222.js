...
app.post('/essaie', async (req, res) => {
  try {
    ...
    // Utilisation de `db.query` pour mettre à jour les informations de l'utilisateur
    const query = 'UPDATE utilisateurs SET essaie = ?, carte = ?, status = ?, date_debut = ?, date_fin = ? WHERE id = ?';
    const values = ['true', 0, 'active', trialStartDate, trialEndDate, userId];
