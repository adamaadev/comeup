...
app.post('/webhook', bodyParser.raw({ type: 'application/json' }), async (req, res) => {
  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      const plan = session.display_items[0].plan.nickname.toLowerCase(); // 'free', 'starter', 'premium', etc.

      // Déterminer les dates d'abonnement
      const now = new Date();
      let date_debut = now;
      let date_fin;
      ...
      if (plan === 'free') {
        ...
        date_fin.setDate(now.getDate() + 7); // Essai gratuit d'une semaine
      } else if (plan === 'starter') {
        date_fin = new Date(now);
        date_fin.setMonth(now.getMonth() + 1); // Plan mensuel
      } else if (plan === 'premium') {
        date_fin = new Date(now);
        date_fin.setFullYear(now.getFullYear() + 1); // Plan annuel
      }

      // Mettre à jour la table utilisateurs dans la base de données
      const query = `
        UPDATE utilisateurs 
        SET status = 'active', plan = ?, date_debut = ?, date_fin = ?
        WHERE email = ?`;

      db.execute(query, [plan, date_debut, date_fin, email], (err, results) => {
        if (err) {
          console.error('Erreur lors de la mise à jour de l\'abonnement:', err);
          return res.status(500).send('Erreur serveur');
        }

        console.log('Abonnement mis à jour pour l\'utilisateur:', email);
        res.status(200).send('Webhook reçu et traité');
      });
    } else {
      res.status(400).send('Événement non pris en charge');
    }
  } catch (err) {
