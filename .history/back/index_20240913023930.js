...
app.post('/essaie', (req, res) => {
  const { id, essaie, date_debut, date_fin } = req.body;

  // Validation des données (par exemple, vérifier que `id` est un nombre et que `essaie` est bien 'true' ou 'false')
  if (typeof id !== 'number' || (essaie !== 'true' && essaie !== 'false')) {
  ...
  const updateQuery = `
    UPDATE utilisateurs
    SET essaie = ?, date_debut = ?, date_fin = ?
    WHERE id = ?
  `;

  // Exécuter la requête SQL
  connection.query(updateQuery, [essaie, date_debut, date_fin, id], (error, results) => {
    if (error) {
