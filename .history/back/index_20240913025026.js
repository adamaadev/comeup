...
app.post('/essaie', (req, res) => {
  const { id, essaie, date_debut, date_fin, card_number } = req.body;

  // Validation des données
  if (typeof id !== 'number' || (essaie !== 'true' && essaie !== 'false') || !card_number) {
  ...
  const updateQuery = `
    UPDATE utilisateurs
    SET essaie = ?, date_debut = ?, date_fin = ?, card_number = ?
    WHERE id = ?
  `;

  // Exécuter la requête SQL
  con.query(updateQuery, [essaie, date_debut, date_fin, card_number, id], (error, results) => {
    if (error) {
