...
const evaluateRatio = (ratio) => {
  // Convertit la valeur en nombre flottant si possible
  const value = parseFloat(ratio);
  
  if (isNaN(value)) return 'N/A'; // Retourne 'N/A' si la valeur n'est pas un nombre
...
app.get('/api/ratios', (req, res) => {
  ...
  con.query(query, (error, results) => {
    ...
    if (results && results.length > 0) {
      ...
      const notations = {
        croissance_annualisee: evaluateRatio(result.croissance_annualisee),
        croissance_moyenne: evaluateRatio(result.croissance_moyenne),
        debt_equity: evaluateRatio(result.debt_equity),
        ratio_payout: evaluateRatio(result.ratio_payout),
        performance: evaluateRatio(result.performance),
        buyback_yield: evaluateRatio(result.buyback_yield),
        croissance_CA_1_an: evaluateRatio(result.croissance_CA_1_an),
        croissance_CA_5_ans: evaluateRatio(result.croissance_CA_5_ans),
        croissance_CA_10_ans: evaluateRatio(result.croissance_CA_10_ans),
        fcf_1_year: evaluateRatio(result.fcf_1_year),
        fcf_5_years: evaluateRatio(result.fcf_5_years),
        fcf_10_years: evaluateRatio(result.fcf_10_years),
        fcf_margin_one_year: evaluateRatio(result.fcf_margin_one_year.trim()), // Trimmed extra spaces
        fcf_margin_five_year: evaluateRatio(result.fcf_margin_five_year.trim()), // Trimmed extra spaces
        roce: evaluateRatio(result.roce),
        roce_5_year_avg: evaluateRatio(result.roce_5_year_avg),
        croissance_resultat_net_1_an: evaluateRatio(result.croissance_resultat_net_1_an),
        croissance_resultat_net_5_ans: evaluateRatio(result.croissance_resultat_net_5_ans),
        piotroski_score: evaluateRatio(result.piotroski_score),
        ratio_capex_revenu_net: evaluateRatio(result.ratio_capex_revenu_net),
      };
