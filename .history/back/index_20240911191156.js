...
function Update() {
  ...
  con.query("SELECT COUNT(*) AS count FROM info", (err, result) => {
    ...
    if (result[0].count === 0) {
      con.query("INSERT INTO info (symbol, currency, pays) SELECT symbol, currency, pays FROM screener", (err, insertResult) => {
        if (err) throw err;
    ...
    con.query("SELECT symbol, pays, currency FROM info LIMIT 1", (err, result) => {
      if (err) throw err;
                ...
                if (result) {
                  con.query('DELETE FROM info WHERE symbol = ?', [symbol], (err, success) => {
                    if (success) {
                      ...
                      Update(); // Appel récursif pour traiter le prochain symbole
                    }
                  });
                }
              });
...
// Lancer le processus une première fois
Update();
setInterval(() => {
