...
app.post('/essaie', async (req, res) => {
  try {
    ...
    const [userRows] = await db.query('SELECT email FROM utilisateurs WHERE id = ?', [userId]);
    ...
    // Créer la session Stripe avec l'email dans les métadonnées
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: {
        email: userEmail // Inclure l'email dans les métadonnées
      }
    });

    const trialStartDate = new Date();
    const trialEndDate = new Date(trialStartDate);
    trialEndDate.setDate(trialEndDate.getDate() + 7); // Période d'essai de 7 jours

    // Enregistrer les informations de l'essai gratuit
    const query = 'UPDATE utilisateurs SET essaie = ?, status = ?, date_debut = ?, date_fin = ? WHERE id = ?';
    const values = ['true', 'active', trialStartDate, trialEndDate, userId];
