...
export function screener(req, res) {
  ...
  let dataQuery = `SELECT * FROM screener ORDER BY marketcap ${sortOrder} LIMIT ? OFFSET ?`;

  if (filterType && filterValue) {
    totalQuery = `SELECT COUNT(*) as count FROM screener WHERE ${filterType} = ?`;
    dataQuery = `SELECT * FROM screener WHERE ${filterType} = ? ORDER BY marketcap ${sortOrder} LIMIT ? OFFSET ?`;
  }
