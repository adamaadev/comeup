  ...
  con.query("SELECT * FROM utilisateurs WHERE email = ? AND type = ?", [email, type], (err, result) => {
    ...
    if (result.length === 0) {
      ...
      bcrypt.compare(password, result[0].password, (err, match) => {
        ...
        if (match) {
          const { id, username, email, essaie, status, type, carte, date_debut, date_fin, plan } = result[0];
          console.log(result[0]);
          
          // Vérification des dates et mise à jour du statut si nécessaire
          if (date_debut && date_fin && date_debut === date_fin) {
            con.query("UPDATE utilisateurs SET status = 'inactive' WHERE email = ?", [email], (updateErr) => {
              if (updateErr) {
                return res.status(500).send({ message: "Erreur de mise à jour du statut" });
              }else{
                console.log("dsssssssss");
                
              }
              // Renvoie une réponse après la mise à jour du statut
              sendTokenResponse();
            });
          } else {
            sendTokenResponse();
          ...
          function sendTokenResponse() {
            if (type === "user") {
              const token = jwt.sign({ id, username, email, essaie, status, type, carte, date_debut, date_fin, plan }, "token___user", { expiresIn: "6d" });
              res.cookie("token_user", token); 
            } else {
              const token = jwt.sign({ id, username, email, essaie, status, type, carte, date_debut, date_fin, plan  }, "token___admin", { expiresIn: "6d" });
