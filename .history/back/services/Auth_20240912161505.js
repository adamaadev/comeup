  ...
  con.query("SELECT * FROM utilisateurs WHERE email = ? AND type = ?", [email, type], (err, result) => {
    ...
    if (result.length === 0) {
      ...
      bcrypt.compare(password, result[0].password, (err, match) => {
        ...
        if (match) {
          const { id, username, email, type } = result[0];
          if (type === "user") {
            const token = jwt.sign({ id, username, email , essaie }, "token___user", { expiresIn: "14d" });
            res.cookie("token_user", token); 
          } else {
            const token = jwt.sign({ id, username, email }, "token___admin", { expiresIn: "14d" });
