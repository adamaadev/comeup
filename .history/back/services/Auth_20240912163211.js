  ...
  con.query("SELECT * FROM utilisateurs WHERE email = ? AND type = ?", [email, type], (err, result) => {
    ...
    if (result.length === 0) {
      ...
      bcrypt.compare(password, result[0].password, (err, match) => {
        ...
        if (match) {
          const { id, username, email, type , essaie } = result[0];
          console.log( id, username, email, type , essaie);
          
          if (type === "user") {
            const token = jwt.sign({ id, username, email , essaie }, "token___user", { expiresIn: "6d" });
            res.cookie("token_user", token); 
          } else {
            const token = jwt.sign({ id, username, email , essaie }, "token___admin", { expiresIn: "6d" });
            res.cookie("token_admin", token); 
          }
          return res.send({ success: true, id, email , essaie});
        } else {
