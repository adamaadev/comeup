  ...
  con.query("SELECT * FROM utilisateurs WHERE email = ? AND type = ?", [email, type], (err, result) => {
    ...
    if (result.length === 0) {
      ...
      bcrypt.compare(password, result[0].password, (err, match) => {
        ...
        if (match) {
          const { id, username, email, password, essaie, status, type, carte, date_debut, date_fin, plan } = result[0];
          console.log(id, username, email, password, essaie, status, type, carte, date_debut, date_fin, plan);
          
          if (type === "user") {
            const token = jwt.sign({ id, username, email, password, essaie, status, type, carte, date_debut, date_fin, plan }, "token___user", { expiresIn: "6d" });
