    ...
    if (result.length === 0) {
      ...
      bcrypt.compare(password, result[0].password, (err, match) => {
        ...
        if (match) {
          ...
          if (type === "user") {
            const token = jwt.sign({ id, email, essaie, status, type, carte, date_debut, date_fin, plan }, "token___user", { expiresIn: "6d" });
            res.cookie("token_user", token); 
            return res.send({ success: true, id, email, essaie, status, type, carte, date_debut, date_fin, plan});
          } else {
