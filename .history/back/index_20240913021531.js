...
app.post('/essaie', async (req, res) => {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      payment_method_types: ['card'], // Collecte les informations de la carte
      line_items: [{
        price_data: {
          currency: 'usd',
          product_data: {
          ...
          unit_amount: 0, // Montant gratuit pour l'essai
        },
        quantity: 1,
      }],
      mode: 'payment',
      ...
      // customer_email: 'user@example.com' // Optionnel : ajouter l'email de l'utilisateur
    });
