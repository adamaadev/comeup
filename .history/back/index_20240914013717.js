...
app.post("/", CheckUser, (req, res) => {
    res.send({ success: true, id : req.id , username: req.username, email: req.email , status : req.status , essaie : req.essaie , status : req.status , type : req.type, carte, date_debut, date_fin, plan});
});
...
app.post('/create-checkout-session', async (req, res) => {
  try {
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price: 'price_1PqNAaFTPbYwB9q1b2pA90nF', // ID du prix de l'abonnement sur Stripe
          quantity: 1,
        },
      ],
      mode: 'subscription',
      success_url: 'http://localhost:3000/success', // URL après un succès
      cancel_url: 'http://localhost:3000/cancel', // URL après une annulation
    });
