...
app.get('/subscribe', verifyToken, async (req, res) => {
  ...
  try {
      ...
      // Enregistrer l'ID du client Stripe dans la base de données
      const updateSubscriptionQuery = 'UPDATE utilisateurs SET stripe_customer_id = ? WHERE email = ?';
      db.query(updateSubscriptionQuery, [customer.id, user.email], (err) => {
          if (err) {
              return res.status(500).json({ message: 'Erreur lors de l\'enregistrement du client Stripe', error: err });
          }
      });
