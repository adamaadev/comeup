...
app.post('/submitscore',(req,res)=>{
    con.query('SELECT * FROM scores WHERE id_user = ? AND symbol = ?',[req.body.id,req.body.symbol],(err,result)=>{
      if (result.length > 0) {
        con.query('UPDATE scores SET score = ? , symbol = ? WHERE id_user = ?',[req.body.score ,req.body.symbol , req.body.id],(err,result)=>{
         
        })
      }else{
        con.query('INSERT INTO scores VALUES (?,?,?,?)',[null,req.body.id,req.body.symbol,req.body.score ],(err,result)=>{
...
app.use(bodyparser.raw({ type: 'application/json' })); // Pour les webhooks Stripe
...
app.get('/success', async (req, res) => {
  ...
  // Récupérer la session de paiement pour obtenir les détails
  const session = await stripe.checkout.sessions.retrieve(sessionId);

  // Mettre à jour les informations d'abonnement dans la base de données
  const updateSubscriptionQuery = 'UPDATE utilisateurs SET subscription_status = ?, subscription_plan = ?, subscription_start_date = ? WHERE stripe_customer_id = ?';
  db.query(updateSubscriptionQuery, ['active', session.subscription, new Date(), session.customer], (err) => {
      if (err) {
          return res.status(500).json({ message: 'Erreur lors de la mise à jour de l\'abonnement', error: err });
      }

      // Récupérer les informations mises à jour de l'utilisateur, y compris la date de fin d'abonnement
      const query = 'SELECT subscription_end_date FROM utilisateurs WHERE stripe_customer_id = ?';
      db.query(query, [session.customer], (err, results) => {
          if (err) {
              return res.status(500).json({ message: 'Erreur serveur', error: err });
          }

          const subscriptionEndDate = results[0] ? new Date(results[0].subscription_end_date).toLocaleDateString() : null;

          // Rediriger vers la page de succès en passant la date de fin d'abonnement
          res.render('success', { email: session.customer_email, subscriptionEndDate });
      });
  });
});
