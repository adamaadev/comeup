  ...
  useEffect(() => {
    const applyFilters = () => {
      return infos.filter(info => {
        ...
        // Condition 2: Vérifier les symboles contenant un point et appliquer la règle des deux lettres après le point
        const containsDot = symbol.includes('.');
        const containstiret = symbol.includes('-');
        if (containsDot && symbol.split('.')[1].length >= 1) return false;
  
        // Condition 3: Afficher uniquement les symboles sans point ou ceux qui ont exactement deux lettres après un point
        const matchesQuery = containsDot ? symbol.split('.')[0].includes(queryLower) : symbol.includes(queryLower);
