...
export default function Screener() {
  ...
  const handleQueryChange = (event) => {
    ...
    if (newQuery === '') {
      ...
      const closestMatch = getClosestMatch(newQuery, infos);
      if (closestMatch) {
        setFilteredInfos([closestMatch]); // Afficher uniquement l'entreprise la plus proche
      } else {
        setFilteredInfos([]); // Aucun résultat trouvé
      }
    }
  };
  ...
  const getClosestMatch = (query, infos) => {
    if (!query) return null;
  
    return infos.reduce((closest, info) => {
      const distance = levenshteinDistance(query.toUpperCase(), info.symbol.toUpperCase());
      if (distance < closest.distance) {
        return { info, distance };
      }
      return closest;
    }, { info: null, distance: Infinity }).info;
  };
