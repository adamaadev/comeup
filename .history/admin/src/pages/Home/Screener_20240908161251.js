...
export default function Screener() {
  ...
  useEffect(() => {
    // Vérifier les filtres pour ajouter des colonnes spécifiques
    const filterColumnTypes = ['buyback_yield', 'croissance_CA_1_an', 'croissance_CA_5_ans', 'croissance_CA_10_ans'];
    const newColumns = [...columns];
  
    filterColumnTypes.forEach(ratio => {
      const isFilterApplied = filters.some(filter => filter.filterType === ratio);
      if (isFilterApplied && !newColumns.includes(ratioNames[ratio])) {
        newColumns.push(ratioNames[ratio]);
      }
    });
  
    // Mettre à jour l'état des colonnes si nécessaire
