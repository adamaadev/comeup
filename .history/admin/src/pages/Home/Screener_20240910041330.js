...
export default function Screener() {
  ...
  useEffect(() => {
    const applyFilters = () => {
      return infos.filter(info => {
        ...
        const name = info.Nom.toLowerCase(); // Ajouter la condition pour le nom
        const queryLower = query.toLowerCase();
  
        // Condition 1: Ne pas suggérer si le symbole a plus de 3 lettres en commun avec la requête
        const commonLetters = symbol.split('').filter(char => queryLower.includes(char)).length;
        if (commonLetters > 3) return false;
  
        // Condition 2: Vérifier si le symbole contient un point ou un tiret
        const containsDot = symbol.includes('.');
        const containsTiret = symbol.includes('-');
  
        // Exclure les symboles avec un point et plus d'une lettre après le point
        if (containsDot && symbol.split('.')[1].length >= 1) return false;
  
        // Exclure les symboles avec un tiret si nécessaire (tu peux ajuster cette condition selon tes besoins)
        if (containsTiret) return false;
  
        // Condition 3: Afficher uniquement les symboles ou les noms qui correspondent à la requête
        const matchesQuery = (containsDot || containsTiret 
          ? symbol.split('.')[0].includes(queryLower) 
          : symbol.includes(queryLower)) 
          || name.includes(queryLower); // Vérifier aussi si le nom contient la requête
  ...
  return (
    <section className="container mt-4">
      ...
      <span className="me-3">
        Total entreprises : {filteredInfos.length}
      </span>
      <span className="me-3">
