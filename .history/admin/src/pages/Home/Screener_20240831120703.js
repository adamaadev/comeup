            ...
            <table className="table table-striped">
              <thead>
                <tr>
                  ...
                  <th>PEA</th>
                  <th>Croissance CA 1 an (%)</th>
                  <th>Croissance CA 5 ans (%)</th>
                  <th>Croissance CA 10 ans (%)</th>
                  <th>Croissance Annualisée (%)</th>
                  <th>Croissance Moyenne (%)</th>
                  <th>Croissance Résultat Net 1 an (%)</th>
                  <th>Croissance Résultat Net 5 ans (%)</th>
                </tr>
              </thead>
              <tbody>
                {currentItems.map(company => (
                  <tr key={company.symbol} onClick={() => window.open(`/details/${company.symbol}`, '_blank')}>
                    <td className="d-flex align-items-center">
                      <img src={company.logo} width="50" height="50" style={{ display: 'inline-block', maxWidth: '50px' }} alt={`${company.Name} logo`} />
                    ...
                    <td>{company.croissance_CA_1_an}%</td>
                    <td>{company.croissance_CA_5_ans}%</td>
                    <td>{company.croissance_CA_10_ans}%</td>
                    <td>{company.croissance_annualisee}%</td>
                    <td>{company.croissance_moyenne}%</td>
                    <td>{company.croissance_resultat_net_1_an}%</td>
                    <td>{company.croissance_resultat_net_5_ans}%</td>
                  </tr>
                ))}
              </tbody>
            </table>
            <div className="pagination">
              <button className="btn btn-primary me-2" onClick={handlePrevPage} disabled={currentPage === 1}>Précédent</button>
              <span>Page {currentPage} sur {totalPages}</span>
              <button className="btn btn-primary ms-2" onClick={handleNextPage} disabled={currentPage === totalPages}>Suivant</button>
            </div>
          </>
        ) : (
