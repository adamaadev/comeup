...
export default function Region({ symbol }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const latestData = data.find(item => item["2023-09-30"]);

        if (latestData) {
          const geoData2023 = latestData["2023-09-30"];
          const formattedGeoData = [
            { name: 'Americas Segment', value: geoData2023['Americas Segment'], color: '#0088FE' },
            { name: 'Europe Segment', value: geoData2023['Europe Segment'], color: '#00C49F' },
            { name: 'Greater China Segment', value: geoData2023['Greater China Segment'], color: '#FFBB28' },
            { name: 'Japan Segment', value: geoData2023['Japan Segment'], color: '#FF8042' },
            { name: 'Rest of Asia Pacific Segment', value: geoData2023['Rest of Asia Pacific Segment'], color: '#A28EFF' }
          ];
  ...
  const renderCustomLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, percent, index }) => {
    ...
    return (
      ...
      >
        {`${geoData[index].name}: $${(geoData[index].value / 1e9).toFixed(1)}B`}
      </text>
    );
  };
