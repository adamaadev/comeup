  ...
  useEffect(() => {
    const applyFilters = () => {
      return infos.filter(info => {
        ...
        const name = info.Nom.toLowerCase(); // Utiliser 'Nom' avec 'N' majuscule
        const queryLower = query.toLowerCase();
  
        // Si une requête est saisie, filtrer selon le nom ou le symbole
        if (query) {
          // Condition 1: Vérifier si la requête correspond au symbole ou au nom
          const matchesQuery = symbol.includes(queryLower) || name.includes(queryLower);
          ...
          if (symbol.split('.')[1].length >= 1) return false;
          ...
          // Si aucune requête n'est saisie, appliquer les conditions spécifiques
          const containsDot = symbol.includes('.');
          const containsTiret = symbol.includes('-');
          ...
          // Exclure les symboles avec un tiret si nécessaire
    ...
    console.log('Filtered Symbols:', sortedFiltered.map(info => info.symbol));
  }, [filters, query, infos]);
