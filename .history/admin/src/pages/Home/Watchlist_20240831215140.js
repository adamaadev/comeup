...
export default function Watchlist() {
  ...
  useEffect(() => {
    if (id) {
      // Obtenir les données de la watchlist
  ...
  const handleOnDragEnd = (result) => {
    ...
    // Optionnel : Sauvegarder l'ordre à la demande
    // saveOrder(items);
  };

  const saveOrder = (newOrder) => {
    axios.post('http://localhost:4000/save-order', { id, order: newOrder.map((item, index) => ({ symbol: item.symbol, position: index })) })
  ...
  return (
    <section className="container mt-4">
      ...
      <button className="btn btn-primary mt-4" onClick={handleSaveOrder}>Sauvegarder l'ordre</button>
    </section>
  );
}
