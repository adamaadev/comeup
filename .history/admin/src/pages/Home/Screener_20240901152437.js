    ...
    const applyFilters = () => {
      return infos.filter(info => {
        ...
        const matchesFilters = filters.every(({ filterType, selectedOption, min, max }) => {
          ...
          if (['buyback_yield', 'chiffre_d_affaire_1_an', 'chiffre_d_affaire_5_ans', 'chiffre_d_affaire_10_ans', 'croissance_annualisee', 'croissance_moyenne', 'croissance_resultat_net_1_an', 'croissance_resultat_net_5_ans'].includes(filterType)) {
    ...
    <section className="container mt-4">
      ...
      <Modal show={showModal} onHide={handleCloseModal}>
        ...
        <Modal.Body>
            ...
            <div className="row mb-2" key={index}>
              <div className="col-md-4">
                ...
                >
                  ...
                  <option value="chiffre_d_affaire_1_an">Chiffre d'affaire 1 an</option>
                  <option value="chiffre_d_affaire_5_ans">Chiffre d'affaire 5 ans</option>
                  <option value="chiffre_d_affaire_10_ans">Chiffre d'affaire 10 ans</option>
              ...
              {filter.filterType === 'buyback_yield' || ['chiffre_d_affaire_1_an', 'chiffre_d_affaire_5_ans', 'chiffre_d_affaire_10_ans', 'croissance_annualisee', 'croissance_moyenne', 'croissance_resultat_net_1_an', 'croissance_resultat_net_5_ans'].includes(filter.filterType) ? (
                <div className="col-md-4">
                  <input
                    type="number"
                    className="form-control"
          ...
          <button className="btn btn-primary" onClick={addFilter}>Ajouter un filtre</button>
        </Modal.Body>
        <Modal.Footer>
        ...
        <thead>
          <tr>
            {columns.map((col, index) => (
              <th key={index}>
                {col === 'chiffre_d_affaire_1_an' && 'Chiffre d\'affaire 1 an'}
                {col === 'chiffre_d_affaire_5_ans' && 'Chiffre d\'affaire 5 ans'}
                {col === 'chiffre_d_affaire_10_ans' && 'Chiffre d\'affaire 10 ans'}
                {col === 'croissance_annualisee' && 'Croissance Annualisée'}
                {col === 'croissance_moyenne' && 'Croissance Moyenne'}
                {col === 'croissance_resultat_net_1_an' && 'Croissance Résultat Net 1 an'}
                {col === 'croissance_resultat_net_5_ans' && 'Croissance Résultat Net 5 ans'}
                {col !== 'chiffre_d_affaire_1_an' && col !== 'chiffre_d_affaire_5_ans' && col !== 'chiffre_d_affaire_10_ans' && col !== 'croissance_annualisee' && col !== 'croissance_moyenne' && col !== 'croissance_resultat_net_1_an' && col !== 'croissance_resultat_net_5_ans' ? col : null}
              </th>
            ))}
          </tr>
        </thead>
        <tbody>
          {currentItems.map((item, index) => (
            <tr key={index}>
              ...
              {columns.includes('marketcap') && <td>{formatNumber(item.marketcap)} milliards</td>}
              {columns.includes('chiffre_d_affaire_1_an') && <td>{item.chiffre_d_affaire_1_an}</td>}
              {columns.includes('chiffre_d_affaire_5_ans') && <td>{item.chiffre_d_affaire_5_ans}</td>}
              {columns.includes('chiffre_d_affaire_10_ans') && <td>{item.chiffre_d_affaire_10_ans}</td>}
              {columns.includes('croissance_annualisee') && <td>{item.croissance_annualisee}</td>}
              {columns.includes('croissance_moyenne') && <td>{item.croissance_moyenne}</td>}
              {columns.includes('croissance_resultat_net_1_an') && <td>{item.croissance_resultat_net_1_an}</td>}
              {columns.includes('croissance_resultat_net_5_ans') && <td>{item.croissance_resultat_net_5_ans}</td>}
            </tr>
          ))}
        </tbody>
      </table>
