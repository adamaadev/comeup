...
export default function Watchlist() {
  ...
  useEffect(() => {
    const applyFilters = () => {
      const uniqueSymbols = new Set(); // Créer un ensemble pour suivre les symboles uniques
  ...
  const filtered = infos.filter(info => {
    ...
    if (matchesQuery && matchesFilters) {
      // Ajouter le symbole au Set et vérifier s'il est déjà présent
  ...
  // Assurez-vous que `filtered` est défini avant d'essayer de le trier
