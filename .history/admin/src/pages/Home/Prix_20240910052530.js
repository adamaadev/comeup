...
export default function Prix ({symbol}) {
  ...
  const fetchData = async (range) => {
    ...
    try {
      ...
      // Filter data for the last 24 hours if range is 1d
      if (range === '1d') {
        const now = new Date();
        const oneDayAgo = new Date(now - (24 * 60 * 60 * 1000));
        data = data.filter(item => new Date(item.date) >= oneDayAgo);
        // Set previousPrice and currentPrice based on available data
        if (data.length > 0) {
          setPreviousPrice(data.length > 1 ? data[data.length - 2].close : null);
          setCurrentPrice(data[data.length - 1].close);
        }
      } else {
        // Apply existing filters for other ranges
        switch (range) {
          case '1y':
        ...
        setPreviousPrice(data[data.length - 2]?.close || null);
  ...
  return (
    <div style={{ backgroundColor: '#ffffff', color: '#000', padding: '20px', borderRadius: '10px' }}>
      <h2 style={priceStyle}>
        Prix actuel: {currentPrice} USD {previousPrice !== null && (currentPrice > previousPrice ? `(+${(currentPrice - previousPrice).toFixed(2)})` : `(${(currentPrice - previousPrice).toFixed(2)})`)}
      </h2>

      <Tab.Container defaultActiveKey="1y">
        <Nav variant="tabs">
          <Nav.Item>
            <Nav.Link eventKey="1d" onClick={() => setTimeRange('1d')}>1 jour</Nav.Link>
          </Nav.Item>
          <Nav.Item>
