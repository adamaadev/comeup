...
export default function Screener() {
  ...
  const [filters, setFilters] = useState([{ filterType: '', selectedOption: '' }]);
  ...
  const handleFilterChange = (index, field, value) => {
    ...
    // Si le filtre est modifié (pas une option), réinitialiser l'option sélectionnée
    if (field === 'filterType') {
  ...
  const getFilterOptions = (filterType) => {
    ...
    if (filterType === 'buyback_yield') return ['0%', '1-5%', '5-10%', '10%+']; // Options pour buyback_yield
    return [...new Set(infos.map(info => info[filterType]))];
  };

  const filteredInfos = infos.filter(info => {
    return filters.every(({ filterType, selectedOption }) => {
      if (!filterType || !selectedOption) return true;
      ...
      if (filterType === 'buyback_yield') {
        ...
        switch (selectedOption) {
          case '0%':
            return buyback === 0;
          case '1-5%':
            return buyback > 0 && buyback <= 5;
          case '5-10%':
            return buyback > 5 && buyback <= 10;
          case '10%+':
            return buyback > 10;
          default:
            return true;
