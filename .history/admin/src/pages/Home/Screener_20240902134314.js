...
export default function Screener() {
  ...
  useEffect(() => {
    axios.get('http://localhost:4000/screener')
      .then(res => {
        const sortedData = res.data.sort((a, b) => b.marketcap - a.marketcap); // Tri décroissant par marketcap
        setInfos(sortedData);
          ...
          {filters.map((filter, index) => (
            <div className="mb-3" key={index}>
              <div className="d-flex">
                ...
                <Button variant="danger" className="ms-2" onClick={() => removeFilter(index)}>
                  X
                </Button>
              </div>
            </div>
          ...
          <thead>
            <tr>
              {columns.map((column, index) => (
                <th key={index} style={{ minWidth: '120px' }}>{column}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {currentItems.map((info, index) => (
              <tr key={index}>
                ...
                {filters.map((filter, i) => (
                  <td key={i}>{info[filter.filterType]}</td>
                ))}
              </tr>
            ))}
          </tbody>
