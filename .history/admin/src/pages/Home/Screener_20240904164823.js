...
export default function Screener() {
  ...
  const [columns, setColumns] = useState(['Name', 'pays', 'secteur', 'industrie', 'Capitalisation (MDS)']);
  ...
  useEffect(() => {
    axios.get('http://localhost:4000/screener')
      .then(res => {
        ...
        const sortedData = res.data.sort((a, b) => b.Capitalisation (MDS) - a.Capitalisation (MDS)); // Décroissant
        setInfos(sortedData);
  ...
  useEffect(() => {
    const initialColumns = ['Nom', 'pays', 'secteur', 'industrie', 'Capitalisation (MDS) (MDS)', 'exchangeShortName'];
    const newColumns = [
  ...
  useEffect(() => {
    ...
    const sortedFiltered = filtered.sort((a, b) => b.Capitalisation (MDS) - a.Capitalisation (MDS));
