...
export default function Screener() {
  ...
  useEffect(() => {
    const filterOptions = (filterType) => {
      ...
      return [...new Set(infos.map(info => info[filterType]))].filter(option => !filteredInfos.some(info => info[filterType] === option));
    };

    setFilters(filters.map(filter => ({
      ...filter,
      filterOptions: filterOptions(filter.filterType)
    })));
  }, [infos, filteredInfos, filters]);
  ...
  const paginatedInfos = filteredInfos.slice(startIndex, startIndex + itemsPerPage);
  ...
  return (
    <div>
      <div>
        ...
        />
        <button onClick={handleModalShow}>Ajouter un filtre</button>
      </div>
      ...
      <div>
        {paginatedInfos.map((info, index) => (
          <div key={index}>
            {columns.map(column => (
              <span key={column}>{info[ratioNames[column]] || ''}</span>
            ))}
          </div>
        ))}
      </div>
      ...
      <div>
        {Array.from({ length: Math.ceil(filteredInfos.length / itemsPerPage) }, (_, i) => (
          <button
            key={i + 1}
      ...
      <Modal show={showModal} onHide={handleCloseModal}>
        ...
        <Modal.Body>
          {filters.map((filter, index) => (
            <div key={index}>
              ...
              >
                ...
                <option value="croissance_resultat_net_1_an">Résultat Net 1 an</option>
                <option value="croissance_resultat_net_5_ans">Résultat Net 5 ans</option>
                <option value="croissance_CA_1_an">CA 1 an</option>
                <option value="croissance_CA_5_ans">CA 5 ans</option>
                <option value="croissance_CA_10_ans">CA 10 ans</option>
          ...
          <button onClick={addFilter}>Ajouter un autre filtre</button>
        </Modal.Body>
        <Modal.Footer>
