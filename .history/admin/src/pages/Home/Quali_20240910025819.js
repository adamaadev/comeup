...
export default function Quali({ symbol }) {
  ...
  const calculateQuantiScore = (data) => {
    const ratios = [
      ...
      data.new_ratio_1,  // Add these new ratios as needed
      data.new_ratio_2,
      data.new_ratio_3,
      data.new_ratio_4
    ];
    ...
    const score = ratios.reduce((total, ratio) => {
      if (ratio === undefined || ratio === null) return total; // Handle undefined or null values
    ...
    <div>
      ...
      <div className="grid-container">
        ...
        <ul>
          ...
          <li>New Ratio 1 : {infos.new_ratio_1}</li>
          <li>New Ratio 2 : {infos.new_ratio_2}</li>
          <li>New Ratio 3 : {infos.new_ratio_3}</li>
          <li>New Ratio 4 : {infos.new_ratio_4}</li>
        </ul>
      </div>
    </div>
  );
