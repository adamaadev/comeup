...
export default function Screener() {
  ...
  const [columns, setColumns] = useState([
    'Name', 'pays', 'secteur', 'industrie', 'marketcap' // Initial columns
  ]);
  ...
  useEffect(() => {
    const newColumns = ['Name', 'pays', 'secteur', 'industrie', 'marketcap'];

    // Ajoute la colonne "Chiffre d'affaire 1 an" si le filtre est appliqué
    if (filters.some(f => f.filterType === 'croissance_CA_1_an')) {
      newColumns.push('Chiffre d\'affaire 1 an');
    }
    ...
    if (secondSelectChanged) {
      const filtered = infos.filter(info => {
        return filters.every(({ filterType, selectedOption, min, max }) => {
          ...
          if (['croissance_CA', 'croissance_CA_5_ans', 'croissance_CA_10_ans', 'croissance_annualisee', 'croissance_moyenne', 'croissance_resultat_net_1_an', 'croissance_resultat_net_5_ans'].includes(filterType)) {
          ...
          <tbody>
            {currentItems.map((item, index) => (
              <tr key={index}>
                ...
                {filters.some(f => f.filterType === 'croissance_CA_1_an') && (
                  <td>{item.croissance_CA_1_an}</td>
                )}
                {filters.map((filter, index) => (
                  filter.filterType && !['croissance_CA_1_an'].includes(filter.filterType) ? (
                    <td key={index}>{item[filter.filterType]}</td>
                  ) : null
                ))}
              </tr>
            ))}
