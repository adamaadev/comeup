...
export default function Screener() {
  ...
  const [columns, setColumns] = useState(['Name', 'pays', 'secteur', 'industrie', 'marketcap', 'croissance_CA_1_an']);
  ...
  useEffect(() => {
    const newColumns = ['Name', 'pays', 'secteur', 'industrie', 'marketcap', 'Chiffre d\'affaire 1 an', ...filters.map(f => f.filterType).filter(f => f)];
          ...
          <tbody>
            {currentItems.map((item, index) => (
              <tr key={index}>
                ...
                <td>{item.croissance_CA_1_an}</td> {/* Chiffre d'affaire 1 an */}
                {filters.map((filter, index) => (
