...
export default function Screener() {
  ...
  const handleFilterChange = (index, field, value) => {
    ...
    addMissingRatios(value); // Ajoutez cette ligne pour ajouter les ratios manquants
  };
  ...
  const addMissingRatios = (filterType) => {
    if (filterType && !infos.some(info => info.hasOwnProperty(filterType))) {
      axios.get(`http://localhost:4000/screener?ratio=${filterType}`)
        .then(res => {
          setInfos(prevInfos => [...prevInfos, ...res.data]);
        });
    }
  };
