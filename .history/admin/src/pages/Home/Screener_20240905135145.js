...
export default function Screener() {
  ...
  useEffect(() => {
    // Charger les colonnes depuis localStorage si elles existent
    const savedColumns = localStorage.getItem('columnsOrder');
  ...
  useEffect(() => {
    const saveColumnsBeforeUnload = () => {
      localStorage.setItem('columnsOrder', JSON.stringify(columns));
    };
  
    window.addEventListener('beforeunload', saveColumnsBeforeUnload);
  
    return () => {
      window.removeEventListener('beforeunload', saveColumnsBeforeUnload);
    };
  }, [columns]);
