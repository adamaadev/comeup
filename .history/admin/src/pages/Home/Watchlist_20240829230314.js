...
export default function Watchlist() {
    ...
    if (id) {
      axios.post('http://localhost:4000/watchlist', { id, type })
        .then(res => {
          ...
          const translatedCompaniesPromises = companies.map(company => 
            translateCountry(company.Country).then(translatedCountry => {
              return { ...company, Country: translatedCountry };
            })
          );
          Promise.all(translatedCompaniesPromises).then(setInfos);
        });
    }
  }, [id]);
  ...
  const translateCountry = async (country) => {
    try {
      ...
      return country; // Retourne le pays original en cas d'erreur
    }
  };
            ...
            <tbody>
              {filteredInfos.map(company => (
               <tr key={company.symbol} onClick={() => window.open(`/details/${company.symbol}`, '_blank')}>
                  ...
                  <td>{company.dividends ? 'Oui' : 'Non'}</td>
               </tr>
              ))}
            </tbody>
          </table>
