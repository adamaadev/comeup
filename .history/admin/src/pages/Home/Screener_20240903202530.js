...
export default function Screener() {
  ...
  const [columns, setColumns] = useState(['Name', 'pays', 'secteur', 'industrie', '
marketCap
']);
  ...
  useEffect(() => {
    axios.get('http://localhost:4000/screener')
      .then(res => {
        ...
        const sortedData = res.data.sort((a, b) => b.
marketCap
 - a.
marketCap
); // Décroissant
  ...
  useEffect(() => {
    const initialColumns = ['Name', 'pays', 'secteur', 'industrie', '
marketCap
', 'exchangeShortName'];
    const newColumns = [
  ...
  useEffect(() => {
    ...
    if (secondSelectChanged) {
      ...
      const sortedFiltered = filtered.sort((a, b) => b.
marketCap
 - a.
marketCap
);
