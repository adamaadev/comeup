...
export default function Screener() {
  ...
  const additionalResults = 3; // Nombre d'entreprises supplémentaires à afficher
  ...
  useEffect(() => {
    const applyFilters = () => {
      return infos.filter(info => {
        const matchesQuery = info.symbol.toLowerCase() === query.toLowerCase();
    ...
    // Mise à jour des résultats de l'autocomplete
    const autoCompleteFiltered = filtered.slice(0, itemsPerPage + additionalResults);
    setAutocompleteResults(autoCompleteFiltered);
  }, [filters, query, infos]);
