      ...
      {isSidebarOpen && (
        <aside className={`bg-light border-start p-3 position-fixed top-0 end-0 vh-100 sidebar ${isSidebarOpen ? 'open' : ''}`} style={{ minWidth: '250px', zIndex: '1000' }}>
          ...
          <nav className="d-flex flex-column mt-5">
            ...
            <NavLink to={'/compte'} style={{ textDecoration: 'none' }} className="mb-3">Emails Clients</NavLink>
            ...
            <div className="d-flex align-items-center position-relative">
              <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="btn btn-link p-0 d-flex align-items-center" style={{ textDecoration: 'none' }}>
                ...
                <div className="position-absolute top-100 end-0 bg-white shadow rounded mt-2">
                 ...
                 i <button onClick={handleLogout} className="dropdown-item btn btn-link" style={{ textDecoraton: 'none' }}>Se déconnecter</button>
                </div>
            </div>
          </nav>
        </aside>
