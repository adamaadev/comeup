  ...
  useEffect(() => {
    const applyFilters = () => {
      return infos.filter(info => {
        // Vérifie si le symbole est strictement égal à la requête (sans variantes)
        const matchesQuery = info.symbol.toLowerCase() === query.toLowerCase() || info.Nom.toLowerCase().includes(query.toLowerCase());
