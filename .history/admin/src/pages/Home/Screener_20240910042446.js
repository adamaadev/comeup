  ...
  useEffect(() => {
    const applyFilters = () => {
      return infos.filter(info => {
        ...
        if (query) {
          // Si une requête est saisie, filtrer les entreprises selon la requête
          const matchesQuery = symbol.includes(queryLower);
          ...
          // Exclure les symboles avec un tiret si nécessaire (ajustez cette condition selon vos besoins)
          if (containsTiret) return false;
  
          // Condition 3: Vérifier les filtres supplémentaires
