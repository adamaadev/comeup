        ...
        {activeTab === 'Informations' && (
          <div className="d-flex">
            ...
            <div className="info-section">
              <h4>Informations</h4>
              <p>Ticker : </strong> {infos.symbol}</p>
              <p>Secteur & Industrie : </strong>{infos.secteur} - {infos.industrie}</p>
                <p>Site web : </strong><a href={infos.website} target="_blank" rel="noopener noreferrer">{infos.website}</a></p>
                <p>Capitalisation : </strong> {infos.marketcap ? formatNumber(infos.marketcap) : 'N/A'}</p>
                <p>Prix : </strong> {infos.price}</p>
                <p>Exchange : </strong> {infos.exchangeShortName}</p>
                <p>Pays : </strong> {infos.pays}</p>
                <p>Isin : </strong> {infos.isin}</p>
                <p>Éligible au PEA : </strong> {infos.eligiblePea ? 'Oui' : 'Non'}</p>
                <p>Verse Dividende : </strong> {infos.verseDividende ? 'Oui' : 'Non'}</p>
            </div>
          </div>
        )}
        {activeTab === 'quant' && (
