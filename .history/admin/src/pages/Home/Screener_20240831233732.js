...
export default function Screener() {
  ...
  const [secondSelectChanged, setSecondSelectChanged] = useState(false); // État pour suivre le changement du deuxième select
  ...
  const handleFilterChange = (index, field, value) => {
    ...
    if (field === 'filterType') {
      ...
      setSecondSelectChanged(false); // Réinitialiser l'état lorsqu'on change le premier select
    }

    if (field === 'selectedOption' || field === 'min' || field === 'max') {
      setSecondSelectChanged(true); // Activer l'état lorsqu'on change le deuxième select ou les valeurs min/max
    }
