...
export default function Region() {
  ...
  const fetchProductData = async () => {
    try {
      ...
      const formattedProductData = productStats.map(item => {
        const [date, values] = Object.entries(item)[0];
        return {
          year: date,
          ...values
        ...
        <div className="chart-wrapper">
          ...
          <ResponsiveContainer width="100%" height={400}>
            <BarChart data={productData}>
              ...
              <Tooltip />
              <Legend />
              <Bar dataKey="Mac" fill="#0088FE" />
              <Bar dataKey="Service" fill="#00C49F" />
              <Bar dataKey="Wearables, Home and Accessories" fill="#FFBB28" />
              <Bar dataKey="iPad" fill="#FF8042" />
              <Bar dataKey="iPhone" fill="#FF6F61" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
