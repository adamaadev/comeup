  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log(geoResponse.data); // Ajoutez ceci pour voir la structure des données
  
        const geoDataObj = geoResponse.data[0];
        const geoData = geoDataObj ? geoDataObj['2023-09-30'] : null;
        ...
        if (geoData) {
          ...
          throw new Error("GeoData for the specified date not available");
        }
  
        // Fetch product revenue data
        const productResponse = await axios.get(`https://financialmodelingprep.com/api/v4/revenue-product-segmentation?symbol=${symbol}&structure=flat&period=annual&apikey=7DpR3qkhqfZy2qfQDZZwHrz0Ohd6oRUX`);
