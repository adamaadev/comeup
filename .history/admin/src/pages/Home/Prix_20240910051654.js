...
export default function Prix({ symbol }) {
  ...
  const fetchData = async (range) => {
    ...
    try {
      ...
      if (range === '1d') {
        data = data.slice(-1); // Get only the most recent day's data
      } else {
        switch (range) {
          case '1y':
      ...
      setPreviousPrice(data.length > 1 ? data[data.length - 2].close : 0);
      setCurrentPrice(data.length > 0 ? data[data.length - 1].close : 0);
      setLoading(false);
    } catch (err) {
      setError(err);
      setLoading(false);
