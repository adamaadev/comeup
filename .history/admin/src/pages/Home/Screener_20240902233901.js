...
export default function Screener() {
  ...
  useEffect(() => {
    ...
    setColumns([...new Set(newColumns)]); // Éviter les doublons
  }, [filters]);
  ...
  return (
    <section className="container mt-4">
      ...
      <table className="table">
        <thead>
          <tr>
            {columns.map((col, index) => <th key={index}>{ratioNames[col] || col}</th>)}
          </tr>
        </thead>
        <tbody>
          {currentItems.map((item, index) => (
            <tr key={index}>
              {columns.map((col, i) => <td key={i}>{item[col]}</td>)}
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <div className="d-flex justify-content-between">
        <Button variant="secondary" onClick={handlePrevPage} disabled={currentPage === 1}>Précédent</Button>
        <span>Page {currentPage} sur {totalPages}</span>
        <Button variant="secondary" onClick={handleNextPage} disabled={currentPage === totalPages}>Suivant</Button>
      </div>
            ...
            <div className="mb-3" key={index}>
              <div className="mb-2">
                ...
                >
                  ...
                  {/* Ajoutez d'autres options de filtre si nécessaire */}
                </select>
              </div>
              {filter.filterType && (
                <>
              ...
              <Button variant="danger" onClick={() => removeFilter(index)}>Supprimer ce filtre</Button>
            </div>
          ))}
