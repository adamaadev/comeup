...
export default function Screener() {
  ...
  const getSuggestions = (value) => {
    ...
    return inputLength === 0 ? [] : infos.filter(
      info =>
        info.Name.toLowerCase().includes(inputValue) ||
        info.symbol.toLowerCase().includes(inputValue)
    );
  };
  ...
  const inputProps = {
    placeholder: 'Rechercher par symbol ou nom',
    value: query,
        ...
        <tbody>
          {currentItems.map(info => (
            <tr key={info.Name}>
              ...
              {filters.map((filter, index) => (
                <td key={index}>
                  {filter.filterType === 'eligiblePea' ? (info[filter.filterType] ? 'Oui' : 'Non') : info[filter.filterType]}
                </td>
              ))}
            </tr>
          ))}
