...
export default function Watchlist() {
  ...
  useEffect(() => {
    const applyFilters = () => {
      // Filtrer les informations selon la requête de recherche et les filtres appliqués
      ...
      filtered.forEach(info => {
        if (query && info.symbol.toLowerCase() === query.toLowerCase()) {
          if (!uniqueSymbols.has(info.symbol)) {
            uniqueSymbols.set(info.symbol, info);
          } else {
            // Compare suffixes to select the most relevant
            const existing = uniqueSymbols.get(info.symbol);
      ...
      const uniqueFiltered = Array.from(uniqueSymbols.values());
