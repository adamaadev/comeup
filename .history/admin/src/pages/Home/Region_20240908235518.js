  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        useEffect(()=>{
          axios.post('http://localhost:4000/getlogo', { symbol })
          .then(res => {});
        },[])
        // Fetch logo
        const logoResponse = await axios.get('https://financialmodelingprep.com/api/v3/profile/AAPL?apikey=7DpR3qkhqfZy2qfQDZZwHrz0Ohd6oRUX');
        const imageUrl = logoResponse.data[0].image;
        setLogo(imageUrl);
      } catch (err) {
        setError(err);
      } finally {
