...
export default function Screener() {
  ...
  useEffect(() => {
    axios.get('http://localhost:4000/screener')
      .then(res => {
        ...
        setInfos(sortedData);
        setFilteredInfos(sortedData); // Initialement, afficher toutes les données
      });
  }, []);
  ...
  const handleQueryChange = (event) => {
    ...
    if (inputValue.length > 2) {
      const filteredSuggestions = infos.filter(info =>
        ...
        info.symbol.toLowerCase().includes(inputValue.toLowerCase())
      ).map(info => info.Name);

      setSuggestions(filteredSuggestions);
      // Mettre à jour le tableau avec les suggestions filtrées
      setFilteredInfos(filteredSuggestions.length > 0
        ? infos.filter(info => filteredSuggestions.includes(info.Name))
        : infos
      );
    } else {
      setSuggestions([]);
      setFilteredInfos(infos); // Réinitialiser les filtres lorsque la recherche est vide
    }
  };
  ...
  const handleSuggestionClick = (suggestion) => {
    setQuery(suggestion);
    setSuggestions([]);
    setFilteredInfos(infos.filter(info => info.Name === suggestion));
  };
        ...
        <tbody>
          {currentItems.map((item, index) => (
            <tr key={index}>
              {columns.map((column, i) => (
                <td key={i}>
                  {column === 'marketcap' ? formatNumber(item[column]) + ' B' : item[column]}
                </td>
              ))}
            </tr>
          ))}
