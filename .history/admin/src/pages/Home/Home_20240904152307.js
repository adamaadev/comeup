...
import { FaChevronDown, FaChevronUp, FaBars, FaTimes } from 'react-icons/fa'; // Ajout de l'icône de fermeture
  ...
  return (
    <div className="d-flex">
      <div className="flex-grow-1 p-3">
        <header className="d-flex justify-content-between align-items-center py-3" style={{ position: 'fixed', top: 0, width: '100%', backgroundColor: '#fff', zIndex: 1000 }}>
        ...
        <div style={{ marginTop: '100px' }}> {/* Espace sous le header fixe */}
          <Outlet />
        </div>
      </div>
      {isSidebarOpen && ( // Afficher le sidebar uniquement si isSidebarOpen est true
        <aside className={`bg-light border-start p-3 position-fixed top-0 end-0 vh-100 sidebar ${isSidebarOpen ? 'open' : ''}`} style={{ minWidth: '250px', zIndex: '1000' }}>
          <button onClick={() => setIsSidebarOpen(false)} className="btn btn-link p-0 position-absolute top-0 end-0 m-3">
            <FaTimes size={30} /> {/* Icône de fermeture */}
          </button>
