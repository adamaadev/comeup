...
export default function App() {
  ...
  const [otpSubmitted, setOtpSubmitted] = useState(false); // État pour suivre la soumission de l'OTP
  ...
  useEffect(() => {
    ...
    // Vérifie l'état de soumission de l'OTP depuis le localStorage
    if (localStorage.getItem('otpSubmitted') === 'true') {
  ...
  return (
    <BrowserRouter>
      <Routes>
        ...
        <Route path="/code" element = {<Otp setOtpSubmitted={setOtpSubmitted} />} /> 
        <Route path="/signin" element={<Signin />} />
        <Route path="/signup" element={auth ? <Navigate to="/" /> : <Signup />} />
        <Route path="/forget" element={<Forget />} />
        <Route path="/changepassword" element={<ChangePassword />} />
        <Route path="/recovery" element={otpSubmitted ? <Recovery /> : <Navigate to="/code" />} /> {/* Protection de la route */}
