...
export default function Screener() {
  ...
  const fetchCompanies = (page) => {
    axios.get(`http://localhost:4000/listcompany?page=${page}&limit=10`)
      .then(res => {
        setInfos(res.data.data); // Assurez-vous que la structure de votre réponse correspond
        setTotalPages(res.data.totalPages); // Assurez-vous que la structure de votre réponse correspond
      })
