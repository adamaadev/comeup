...
const totalScore = netIncomeCount + operatingCashFlowCount + roaCount + cptnetoperating + cptcurrentratio + cptlongtermdebttotalasset + cptsharesoutstanding + cptmargebrute;
 ...
 useEffect(() => {
  ...
  return (
    <div>   
      {isNaN(buyback) ? 'okkkkkkkkkkkkkkkkkkkk' : <p>Buyback yield 1 an : {buyback} %</p>}
      {isNaN(roce23) ? 'okkkkkkkkkkkkkkkkkkkk' : <p>Roce 1 an : {roce23} %</p>}
      {isNaN(pct1) ? 'okkkkkkkkkkkkkkkkkkkk' : <p>Roce 5 ans : {pct1} %</p>}
      {isNaN(cagr5Years) ? 'okkkkkkkkkkkkkkkkkkkk' : <p>Performance moyenne sur les 5 dernières années : {cagr5Years}%</p>}
      {isNaN(payoutRatio) ? 'okkkkkkkkkkkkkkkkkkkk' : <p>Payout Ratio de {symbol} : {payoutRatio}% </p>}
      {isNaN(fcfMarginOneYear) ? 'okkkkkkkkkkkkkkkkkkkk' : <p>Free Cash Flow Margin 1 an : {fcfMarginOneYear}%</p>}
      {isNaN(averageMarginFiveYears) ? 'okkkkkkkkkkkkkkkkkkkk' : <p>Free Cash Flow Margin 5 ans : {averageMarginFiveYears}%</p>}
      {isNaN(annualizedGrowthRate) ? 'okkkkkkkkkkkkkkkkkkkk' : <p>Croissance Annualisée du Dividende : {annualizedGrowthRate}%</p>}
      {isNaN(netIncomeGrowthOneYear) ? 'okkkkkkkkkkkkkkkkkkkk' : <p>Croissance du résultat net sur 1 an: {netIncomeGrowthOneYear}%</p>}
      {isNaN(netIncomeGrowthFiveYears) ? 'okkkkkkkkkkkkkkkkkkkk' : <p>Croissance du résultat net sur 5 ans: {netIncomeGrowthFiveYears}%</p>}
      {isNaN(averageRatio) ? 'okkkkkkkkkkkkkkkkkkkk' : <p>Capex / Résultat net (moyenne 5 ans) : {averageRatio}%</p>}
      {isNaN(debtToEquityRatio) ? 'okkkkkkkkkkkkkkkkkkkk' : <p>Debt to Equity Ratio de : {debtToEquityRatio}</p>}
      {isNaN(netSharesRepurchases) ? 'okkkkkkkkkkkkkkkkkkkk' : <p>Rachat net moyen d'actions (moyenne 5 ans) {symbol}: {netSharesRepurchases}</p>}
      {isNaN(growthRate) ? 'okkkkkkkkkkkkkkkkkkkk' : <p>Croissance moyenne du dividende sur 5 ans {symbol}: {growthRate} %</p>}
      {isNaN(currentStreak) ? 'okkkkkkkkkkkkkkkkkkkk' : <p> Le dividende n'a pas diminué depuis {currentStreak} an(s)</p>}
      {isNaN(cagr1Year1) ? 'okkkkkkkkkkkkkkkkkkkk' : <p>FCF 1 an : {`${cagr1Year1}%`}</p>}
      {isNaN(cagr5Years2) ? 'okkkkkkkkkkkkkkkkkkkk' :  <p>FCF sur 5 ans : {`${cagr5Years2}%`}</p>}
      {isNaN(cagr10Years3 ) ? 'okkkkkkkkkkkkkkkkkkkk' : <p>FCF sur 10 ans : {`${cagr10Years3}%`}</p>}
      {isNaN(croissanceCA1an) ? 'okkkkkkkkkkkkkkkkkkkk' : <p>Croissance CA 1 an ({dateOneYearAgo} à {dateLastYear}) : {croissanceCA1an}%</p>}
      {isNaN(croissanceCA5ans) ? 'okkkkkkkkkkkkkkkkkkkk' : <p>Croissance CA 5 ans ({dateFiveYearsAgo} à {dateLastYear}) : {croissanceCA5ans}%</p>}
      {isNaN(croissanceCA10ans) ? 'okkkkkkkkkkkkkkkkkkkk' : <p>Croissance CA 10 ans ({dateTenYearsAgo} à {dateLastYear}) : {croissanceCA10ans}%</p>}
