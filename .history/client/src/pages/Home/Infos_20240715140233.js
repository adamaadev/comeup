...
const totalScore = netIncomeCount + operatingCashFlowCount + roaCount + cptnetoperating + cptcurrentratio + cptlongtermdebttotalasset + cptsharesoutstanding + cptmargebrute;
 ...
 useEffect(() => {
  ...
  return (
    <div>   
     {isNaN(buyback) || !isFinite(buyback) || buyback === null ? '' : <p>Buyback yield 1 an : {buyback} %</p>}
    ...
    {isNaN(croissanceCA1an) || !isFinite(croissanceCA1an) || croissanceCA1an === null ? '' : <p>Croissance CA 1 an ({dateOneYearAgo} à {dateLastYear}) : {croissanceCA1an}%</p>}
    {isNaN(croissanceCA5ans) || !isFinite(croissanceCA5ans) || croissanceCA5ans === null ? '' : <p>Croissance CA 5 ans ({dateFiveYearsAgo} à {dateLastYear}) : {croissanceCA5ans}%</p>}
    {isNaN(croissanceCA10ans) || !isFinite(croissanceCA10ans) || croissanceCA10ans === null ? '' : <p>Croissance CA 10 ans ({dateTenYearsAgo} à {dateLastYear}) : {croissanceCA10ans}%</p>}
