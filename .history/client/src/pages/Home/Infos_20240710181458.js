...
const [cptvrotaction , setcptvrotaction] = useState(0);
 useEffect(() => {
  ...
  return (
    <div>
     {(isNaN(App) || isFinite(App) || App === null) ? '' : <p>Buyback yield 1 an {symbol} : {App.toFixed(2)} %</p>}      
     {(isNaN(roce23) || !isFinite(roce23) || roce23 === null) ? '' : <p className="text-muted">Roce 1 an : {roce23} %</p>}
    {(isNaN(pct1) || !isFinite(pct1) || pct1 === null) ? '' : <p className="text-muted">Roce 5 ans : {pct1} %</p>}
    <p className="text-muted">{cagr5Years !== null ? `${cagr5Years}%` : 'Calcul en cours...'}</p>
    {(isNaN(payoutRatio) || !isFinite(payoutRatio) || payoutRatio === null) ? '' : <p className="text-muted">Payout Ratio de {symbol} : {payoutRatio}%</p>}
    {(isNaN(fcfMarginOneYear) || !isFinite(fcfMarginOneYear) || fcfMarginOneYear === null) ? '' : <p className="text-muted">Free Cash Flow Margin 1 an : {fcfMarginOneYear}%</p>}
    {(isNaN(averageMarginFiveYears) || !isFinite(averageMarginFiveYears) || averageMarginFiveYears === null) ? '' : <p className="text-muted">Free Cash Flow Margin 5 ans : {averageMarginFiveYears}%</p>}
    {(isNaN(annualizedGrowthRate) || !isFinite(annualizedGrowthRate) || annualizedGrowthRate === null) ? '' : <p className="text-muted">Croissance Annualisée du Dividende : {annualizedGrowthRate}%</p>}
    {(isNaN(netIncomeGrowthOneYear) || !isFinite(netIncomeGrowthOneYear) || netIncomeGrowthOneYear === null) ? '' : <p className="text-muted">Croissance du résultat net sur 1 an: {netIncomeGrowthOneYear}%</p>}
    {(isNaN(netIncomeGrowthFiveYears) || !isFinite(netIncomeGrowthFiveYears) || netIncomeGrowthFiveYears === null) ? '' : <p className="text-muted">Croissance du résultat net sur 5 ans: {netIncomeGrowthFiveYears}%</p>}
    {(isNaN(netIncomeGrowthTenYears) || !isFinite(netIncomeGrowthTenYears) || netIncomeGrowthTenYears === null) ? '' : <p className="text-muted">Croissance du résultat net sur 10 ans: {netIncomeGrowthTenYears}%</p>}
    {(isNaN(averageRatio) || !isFinite(averageRatio) || averageRatio === null) ? '' : <p className="text-muted">Capex / Résultat net (moyenne 5 ans) : {averageRatio}%</p>}
    {(isNaN(debtToEquityRatio) || !isFinite(debtToEquityRatio) || debtToEquityRatio === null) ? '' : <p className="text-muted">Debt to Equity Ratio de : {debtToEquityRatio}</p>}
    {(isNaN(netSharesRepurchases) || !isFinite(netSharesRepurchases) || netSharesRepurchases === null) ? '' : <p className="text-muted">Rachat net moyen d'actions (moyenne 5 ans) {symbol}: {netSharesRepurchases}</p>}
    {(isNaN(growthRate) || !isFinite(growthRate) || growthRate === null) ? '' : <p className="text-muted">Croissance moyenne du dividende sur 5 ans {symbol}: {growthRate.toFixed(2)} %</p>}
    {(isNaN(currentStreak) || !isFinite(currentStreak) || currentStreak === null) ? '' : <p className="text-muted">La dividende n'a pas diminué depuis {currentStreak} an(s)</p>}
    {(isNaN(cagr1Year1) || !isFinite(cagr1Year1) || cagr1Year1 === null) ? '' : <p className="text-muted">CAGR sur 1 an : {cagr1Year1}</p>}
    {(isNaN(cagr5Years) || !isFinite(cagr5Years) || cagr5Years === null) ? '' : <p className="text-muted">CAGR sur 5 ans : {cagr5Years}</p>}
    {(isNaN(cagr10Years3) || !isFinite(cagr10Years3) || cagr10Years3 === null) ? '' : <p className="text-muted">CAGR sur 10 ans : {cagr10Years3}</p>}
    {(isNaN(totalScore) || !isFinite(totalScore) || totalScore === null) ? '' : <p className="text-muted">Total Piotroski Score: {totalScore} / 9 points</p>}
