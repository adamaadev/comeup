    ...
    if (id) {
      ...
      const fetchExistingQuestions = async () => {
        try {
          ...
          axios.post('http://localhost:4000/checkscore', { id, symbol }).then(res => setScore(res.data.score)); // Mettre à jour le score
  ...
  const handleSubmit = (event) => {
    ...
    axios.post('http://localhost:4000/sendquestion', { id, symbol, modalQuestion, selectedResponse, justification })
      .then(response => {
        ...
        setModalQuestion(null);
        fetchExistingResponses(existingQuestions);
        axios.post('http://localhost:4000/checkscore', { id, symbol }).then(res => setScore(res.data.score)); // Mettre à jour le score après soumission
      })
      .catch(error => {
