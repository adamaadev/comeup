...
const [cptvrotaction , setcptvrotaction] = useState(0);
 useEffect(() => {
  ...
  return (
    <div>   
      {isNaN(buyback) ? 'okkkkkkkkkkk' : <p>Buyback yield 1 an : {buyback} %</p>}
      {isNaN(roce23) ? 'okkkkkkkkkkk' : <p>Roce 1 an : {roce23} %</p>}
      {isNaN(pct1) ? 'okkkkkkkkkkk' : <p>Roce 5 ans : {pct1} %</p>}
      {isNaN(cagr5Years) ? 'okkkkkkkkkkk' : <p>Performance moyenne sur les 5 dernières années : {cagr5Years}%</p>}
      {isNaN(payoutRatio) ? 'okkkkkkkkkkk' : <p>Payout Ratio de {symbol} : {payoutRatio}% </p>}
      {isNaN(fcfMarginOneYear) ? 'okkkkkkkkkkk' : <p>Free Cash Flow Margin 1 an : {fcfMarginOneYear}%</p>}
      {isNaN(averageMarginFiveYears) ? 'okkkkkkkkkkk' : <p>Free Cash Flow Margin 5 ans : {averageMarginFiveYears}%</p>}
      {isNaN(annualizedGrowthRate) ? 'okkkkkkkkkkk' : <p>Croissance Annualisée du Dividende : {annualizedGrowthRate}%</p>}
      {isNaN(netIncomeGrowthOneYear) ? 'okkkkkkkkkkk' : <p>Croissance du résultat net sur 1 an: {netIncomeGrowthOneYear}%</p>}
      {isNaN(netIncomeGrowthFiveYears) ? 'okkkkkkkkkkk' : <p>Croissance du résultat net sur 5 ans: {netIncomeGrowthFiveYears}%</p>}
      {isNaN(averageRatio) ? 'okkkkkkkkkkk' : <p>Capex / Résultat net (moyenne 5 ans) : {averageRatio}%</p>}
      {isNaN(debtToEquityRatio) ? 'okkkkkkkkkkk' : <p>Debt to Equity Ratio de : {debtToEquityRatio}</p>}
      {isNaN(netSharesRepurchases) ? 'okkkkkkkkkkk' : <p>Rachat net moyen d'actions (moyenne 5 ans) {symbol}: {netSharesRepurchases}</p>}
      {isNaN(growthRate) ? 'okkkkkkkkkkk' : <p>Croissance moyenne du dividende sur 5 ans {symbol}: {growthRate} %</p>}
      {isNaN(currentStreak) ? 'okkkkkkkkkkk' : <p> Le dividende n'a pas diminué depuis {currentStreak} an(s)</p>}
      {isNaN(cagr1Year1) ? 'okkkkkkkkkkk' : <p>FCF 1 an : {`${cagr1Year1}%`}</p>}
      {isNaN(cagr5Years2) ? 'okkkkkkkkkkk' :  <p>FCF sur 5 ans : {`${cagr5Years2}%`}</p>}
      {isNaN(cagr10Years3 ) ? 'okkkkkkkkkkk' : <p>FCF sur 10 ans : {`${cagr10Years3}%`}</p>}
      {isNaN(croissanceCA1an) ? 'okkkkkkkkkkk' : <p>Croissance CA 1 an ({dateOneYearAgo} à {dateLastYear}) : {croissanceCA1an}%</p>}
      {isNaN(croissanceCA5ans) ? 'okkkkkkkkkkk' : <p>Croissance CA 5 ans ({dateFiveYearsAgo} à {dateLastYear}) : {croissanceCA5ans}%</p>}
      {isNaN(croissanceCA10ans) ? 'okkkkkkkkkkk' : <p>Croissance CA 10 ans ({dateTenYearsAgo} à {dateLastYear}) : {croissanceCA10ans}%</p>}
    </div>
  )
}