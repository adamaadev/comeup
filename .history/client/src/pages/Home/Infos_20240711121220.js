...
const [cptvrotaction , setcptvrotaction] = useState(0);
 useEffect(() => {
  ...
  return (
    <div>
    <p>Buyback yield 1 an {symbol} : {App?.toFixed(2) ?? 'N/A'} %</p>
<div>
  <p>Roce 1 an : {roce23?.toFixed(2) ?? 'N/A'} %</p>
  <p>Roce 5 ans : {pct1?.toFixed(2) ?? 'N/A'} %</p>
  <p>Performance moyenne sur les 5 dernières années pour {symbol}</p>
  <p>CAGR sur 5 ans : {cagr5Years !== null ? `${cagr5Years.toFixed(2)}%` : 'Calcul en cours...'}</p>
  <p>Payout Ratio de {symbol} : {payoutRatio?.toFixed(2) ?? 'N/A'}% </p>
  <p>Free Cash Flow Margin pour {symbol}</p>
  {fcfMarginOneYear !== null && (
    <p>Free Cash Flow Margin 1 an : {fcfMarginOneYear.toFixed(2)}%</p>
  )}
  <p>Free Cash Flow Margin 5 ans : {averageMarginFiveYears?.toFixed(2) ?? 'N/A'}%</p>
  <p>Croissance Annualisée du Dividende : {annualizedGrowthRate?.toFixed(2) ?? 'N/A'}%</p>
  <p>Croissance du résultat net sur 1 an: {netIncomeGrowthOneYear?.toFixed(2) ?? 'N/A'}%</p>
  <p>Croissance du résultat net sur 5 ans: {netIncomeGrowthFiveYears?.toFixed(2) ?? 'N/A'}%</p>
  <p>Croissance du résultat net sur 10 ans: {netIncomeGrowthTenYears?.toFixed(2) ?? 'N/A'}%</p>
  <p>Capex / Résultat net (moyenne 5 ans) : {averageRatio?.toFixed(2) ?? 'N/A'}%</p>
  <p>Debt to Equity Ratio de : {debtToEquityRatio?.toFixed(2) ?? 'N/A'}</p>
  <p>Rachat net moyen d'actions (moyenne 5 ans) {symbol}: {netSharesRepurchases?.toFixed(2) ?? 'N/A'}</p>
  <p>Croissance moyenne du dividende sur 5 ans {symbol}: {growthRate?.toFixed(2) ?? 'N/A'} %</p>
  <p> Le dividende n'a pas diminué depuis {currentStreak} an(s)</p>
  <p>Calculs de croissance du Free Cash Flow pour {symbol}</p>
  <p>CAGR sur 1 an : {cagr1Year1 !== null ? `${cagr1Year1.toFixed(2)}%` : 'Calcul en cours...'}</p>
  <p>CAGR sur 5 ans : {cagr5Years2 !== null ? `${cagr5Years2.toFixed(2)}%` : 'Calcul en cours...'}</p>
  <p>CAGR sur 10 ans : {cagr10Years3 !== null ? `${cagr10Years3.toFixed(2)}%` : 'Calcul en cours...'}</p>
</div>
