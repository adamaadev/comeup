...
export default function Home() {
  ...
  const fetchSuggestions = (query) => {
    axios.get(`https://financialmodelingprep.com/api/v3/search?query=${query}&limit=10&exchange=NASDAQ&apikey=7DpR3qkhqfZy2qfQDZZwHrz0Ohd6oRUX`)
      .then(response => {
        if (response.data && Array.isArray(response.data)) {
          // Filtrer uniquement les entreprises (non ETFs)
          const filteredCompanies = response.data.filter(company => company.type === 'Common Stock');
