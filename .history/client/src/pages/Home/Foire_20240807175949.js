...
export default function Foire({ symbol }) {
  ...
  useEffect(() => {
    const fetchId = async () => {
      try {
        const response = await axios.post('http://localhost:4000/', { type: "admin" });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        if (id && symbol) {
          const [questionsResponse, scoreResponse] = await Promise.all([
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setJustification("");
      setModalQuestion(null);
      await fetchExistingResponses(existingQuestions);
  ...
  const fetchExistingResponses = async (questions) => {
    // You can add the implementation for fetching existing responses if needed
  };
