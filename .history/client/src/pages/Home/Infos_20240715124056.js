 ...
 useEffect(() => {
  ...
  return (
    <div>   
      ...
      {isNaN(croissanceCA1an) ? '' : <p>Croissance CA 1 an ({dateOneYearAgo} à {dateLastYear}) : {croissanceCA1an}%</p>}
      {isNaN(croissanceCA5ans) ? '' : <p>Croissance CA 5 ans ({dateFiveYearsAgo} à {dateLastYear}) : {croissanceCA5ans}%</p>}
      {isNaN(croissanceCA10ans) ? '' : <p>Croissance CA 10 ans ({dateTenYearsAgo} à {dateLastYear}) : {croissanceCA10ans}%</p>}
    </div>
  )
}