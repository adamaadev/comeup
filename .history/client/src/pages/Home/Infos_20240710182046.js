...
const [cptvrotaction , setcptvrotaction] = useState(0);
 ...
 useEffect(() => {
  ...
  return (
    <div>
        <p><strong>Buyback yield 1 an {symbol} : {App.toFixed(2)} %</strong></p>
        <p><strong>Roce 1 an:</strong> {roce23} %</p>
        <p><strong>Roce 5 ans:</strong> {pct1} %</p>
        <p><strong>Payout Ratio de {symbol}:</strong> {payoutRatio}%</p>
        <p><strong>Free Cash Flow Margin 1 an:</strong> {fcfMarginOneYear}%</p>
        <p><strong>Free Cash Flow Margin 5 ans:</strong> {averageMarginFiveYears}%</p>
        <p><strong>Croissance Annualisée du Dividende:</strong> {annualizedGrowthRate}%</p>
        <p><strong>Croissance du résultat net sur 1 an:</strong> {netIncomeGrowthOneYear}%</p>
        <p><strong>Croissance du résultat net sur 5 ans:</strong> {netIncomeGrowthFiveYears}%</p>
        <p><strong>Croissance du résultat net sur 10 ans:</strong> {netIncomeGrowthTenYears}%</p>
        <p><strong>Capex / Résultat net (moyenne 5 ans):</strong> {averageRatio}%</p>
        <p><strong>Debt to Equity Ratio de:</strong> {debtToEquityRatio}</p>
        <p><strong>Rachat net moyen d'actions (moyenne 5 ans) {symbol}:</strong> {netSharesRepurchases}</p>
        <p><strong>Croissance moyenne du dividende sur 5 ans {symbol}:</strong> {growthRate.toFixed(2)} %</p>
        <p><strong>Le dividende n'a pas diminué depuis:</strong> {currentStreak} an(s)</p>
        <p><strong>CAGR sur 1 an:</strong> {cagr1Year1}</p>
        <p><strong>CAGR sur 5 ans:</strong> {cagr5Years}</p>
        <p><strong>CAGR sur 10 ans:</strong> {cagr10Years3}</p>
        <p><strong>Total Piotroski Score:</strong> {totalScore} / 9 points</p>
    </div>
  )
}