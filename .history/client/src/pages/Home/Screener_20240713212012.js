...
export default function Screener() {
  ...
  const calculateCAGR = (historicalData) => {
            ...
            {filteredInfos.map((company, index) => (
              <tr key={company.id} onClick={() => window.open(`/details/${company.symbol}`, '_blank')}>
                ...
                <td>
                  <span>
                    {(() => {
                      const startDate = `${new Date().getFullYear() - 5}-12-31`;
                      const endDate = `${new Date().getFullYear() - 1}-12-31`;
                      axios.get(`https://financialmodelingprep.com/api/v3/historical-price-full/${company.symbol}?from=${startDate}&to=${endDate}&apikey=7DpR3qkhqfZy2qfQDZZwHrz0Ohd6oRUX`)
                        .then(response => {
                          const historicalData = response.data.historical;
                          const cagr = calculateCAGR(historicalData);
                          return cagr;
                        })
                        .catch(error => {
