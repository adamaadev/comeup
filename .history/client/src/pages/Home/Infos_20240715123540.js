...
const [cptvrotaction , setcptvrotaction] = useState(0);
 useEffect(() => {
  ...
  if (revenues.length < 10 || dates.length < 10) {
    return <p>Loading...</p>;
  }
  ...
  const croissanceCA1an = ((revenueLastYear - revenueOneYearAgo) / revenueOneYearAgo) * 100;
  const croissanceCA5ans = (Math.pow((revenueLastYear / revenueFiveYearsAgo), (1 / 5)) - 1) * 100;
  const croissanceCA10ans = (Math.pow((revenueLastYear / revenueTenYearsAgo), (1 / 10)) - 1) * 100;


  return (
    <div>   
      {isNaN(buyback) ? '' : <p>Buyback yield 1 an : {buyback} %</p>}
      {isNaN(roce23) ? '' : <p>Roce 1 an : {roce23} %</p>}
      {isNaN(pct1) ? '' : <p>Roce 5 ans : {pct1} %</p>}
      {isNaN(cagr5Years) ? '' : <p>Performance moyenne sur les 5 dernières années : {cagr5Years}%</p>}
      {isNaN(payoutRatio) ? '' : <p>Payout Ratio de {symbol} : {payoutRatio}% </p>}
      {isNaN(fcfMarginOneYear) ? '' : <p>Free Cash Flow Margin 1 an : {fcfMarginOneYear}%</p>}
      {isNaN(averageMarginFiveYears) ? '' : <p>Free Cash Flow Margin 5 ans : {averageMarginFiveYears}%</p>}
      {isNaN(annualizedGrowthRate) ? '' : <p>Croissance Annualisée du Dividende : {annualizedGrowthRate}%</p>}
      {isNaN(netIncomeGrowthOneYear) ? '' : <p>Croissance du résultat net sur 1 an: {netIncomeGrowthOneYear}%</p>}
      {isNaN(netIncomeGrowthFiveYears) ? '' : <p>Croissance du résultat net sur 5 ans: {netIncomeGrowthFiveYears}%</p>}
      {isNaN(averageRatio) ? '' : <p>Capex / Résultat net (moyenne 5 ans) : {averageRatio}%</p>}
      {isNaN(debtToEquityRatio) ? '' : <p>Debt to Equity Ratio de : {debtToEquityRatio}</p>}
      {isNaN(netSharesRepurchases) ? '' : <p>Rachat net moyen d'actions (moyenne 5 ans) {symbol}: {netSharesRepurchases}</p>}
      {isNaN(growthRate) ? '' : <p>Croissance moyenne du dividende sur 5 ans {symbol}: {growthRate} %</p>}
      {isNaN(currentStreak) ? '' : <p> Le dividende n'a pas diminué depuis {currentStreak} an(s)</p>}
      {cagr1Year1 !== null ? <p>FCF 1 an : {`${cagr1Year1}%`}</p> : <p>FCF 1 an : Calcul en cours...</p>}
      {cagr5Years2 !== null ? <p>FCF sur 5 ans : {`${cagr5Years2}%`}</p> : <p>FCF sur 5 ans : Calcul en cours...</p>}
      {cagr10Years3 !== null ? <p>FCF sur 10 ans : {`${cagr10Years3}%`}</p> : <p>FCF sur 10 ans : Calcul en cours...</p>}
      {isNaN(croissanceCA1an) ? '' : <p>Croissance CA 1 an ({dateOneYearAgo} à {dateLastYear}) : {croissanceCA1an.toFixed(2)}%</p>}
      {isNaN(croissanceCA5ans) ? '' : <p>Croissance CA 5 ans ({dateFiveYearsAgo} à {dateLastYear}) : {croissanceCA5ans.toFixed(2)}%</p>}
      {isNaN(croissanceCA10ans) ? '' : <p>Croissance CA 10 ans ({dateTenYearsAgo} à {dateLastYear}) : {croissanceCA10ans.toFixed(2)}%</p>}
    </div>
  )
}