      ...
      {isNaN(buyback) || !isFinite(buyback) || buyback === null ? '' :
                <div>
                    ...
                    <div className="progress">
                        <div className={getProgressClass(buyback)} role="progressbar" style={{ width: `${buyback}%` }} aria-valuenow={buyback} aria-valuemin="0" aria-valuemax="100"></div>
                    </div>
                </div>
            }
    {isNaN(roce23) || !isFinite(roce23) || roce23 === null ? '' : <p>Roce 1 an : {roce23} %</p>}
    {isNaN(pct1) || !isFinite(pct1) || pct1 === null ? '' : <p>Roce 5 ans : {pct1} %</p>}
    {isNaN(cagr5Years) || !isFinite(cagr5Years) || cagr5Years === null ? '' : <p>Performance moyenne sur les 5 dernières années : {cagr5Years}%</p>}
    {isNaN(payoutRatio) || !isFinite(payoutRatio) || payoutRatio === null ? '' : <p>Payout Ratio de {symbol} : {payoutRatio}% </p>}
    {isNaN(fcfMarginOneYear) || !isFinite(fcfMarginOneYear) || fcfMarginOneYear === null ? '' : <p>Free Cash Flow Margin 1 an : {fcfMarginOneYear}%</p>}
    {isNaN(averageMarginFiveYears) || !isFinite(averageMarginFiveYears) || averageMarginFiveYears === null ? '' : <p>Free Cash Flow Margin 5 ans : {averageMarginFiveYears}%</p>}
    {isNaN(annualizedGrowthRate) || !isFinite(annualizedGrowthRate) || annualizedGrowthRate === null ? '' : <p>Croissance Annualisée du Dividende : {annualizedGrowthRate}%</p>}
    {isNaN(netIncomeGrowthOneYear) || !isFinite(netIncomeGrowthOneYear) || netIncomeGrowthOneYear === null ? '' : <p>Croissance du résultat net sur 1 an: {netIncomeGrowthOneYear}%</p>}
    {isNaN(netIncomeGrowthFiveYears) || !isFinite(netIncomeGrowthFiveYears) || netIncomeGrowthFiveYears === null ? '' : <p>Croissance du résultat net sur 5 ans: {netIncomeGrowthFiveYears}%</p>}
    {isNaN(averageRatio) || !isFinite(averageRatio) || averageRatio === null ? '' : <p>Capex / Résultat net (moyenne 5 ans) : {averageRatio}%</p>}
    {isNaN(debtToEquityRatio) || !isFinite(debtToEquityRatio) || debtToEquityRatio === null ? '' : <p>Debt to Equity Ratio de : {debtToEquityRatio}</p>}
    {isNaN(netSharesRepurchases) || !isFinite(netSharesRepurchases) || netSharesRepurchases === null ? '' : <p>Rachat net moyen d'actions (moyenne 5 ans) {symbol}: {netSharesRepurchases}</p>}
    {isNaN(growthRate) || !isFinite(growthRate) || growthRate === null ? '' : <p>Croissance moyenne du dividende sur 5 ans {symbol}: {growthRate} %</p>}
    {isNaN(currentStreak) || !isFinite(currentStreak) || currentStreak === null ? '' : <p> Le dividende n'a pas diminué depuis {currentStreak} an(s)</p>}
    {isNaN(cagr1Year1) || !isFinite(cagr1Year1) || cagr1Year1 === null ? '' : <p>FCF 1 an : {`${cagr1Year1}%`}</p>}
    {isNaN(cagr5Years2) || !isFinite(cagr5Years2) || cagr5Years2 === null ? '' :  <p>FCF sur 5 ans : {`${cagr5Years2}%`}</p>}
    {isNaN(cagr10Years3 ) || !isFinite(cagr10Years3) || cagr10Years3 === null ? '' : <p>FCF sur 10 ans : {`${cagr10Years3}%`}</p>}
    {isNaN(croissanceCA1an) || !isFinite(croissanceCA1an) || croissanceCA1an === null ? '' : <p>Croissance CA 1 an : {croissanceCA1an}%</p>}
    {isNaN(croissanceCA5ans) || !isFinite(croissanceCA5ans) || croissanceCA5ans === null ? '' : <p>Croissance CA 5 ans : {croissanceCA5ans}%</p>}
    {isNaN(croissanceCA10ans) || !isFinite(croissanceCA10ans) || croissanceCA10ans === null ? '' : <p>Croissance CA 10 ans : {croissanceCA10ans}%</p>}
