...
export default function Abonnement() {
  ...
  const [pricemensuel, setPricemensuel] = useState('17.99');
  const [priceannuel, setPriceannuel] = useState(13.49 * 12);
  const [selectedPlan, setSelectedPlan] = useState('mensuel'); // Par défaut, le plan est mensuel
  ...
  const handleSubscribe = () => {
    ...
    navigate('/checkoutform', { state: { price: selectedPrice, plan: selectedPlan } });
    setSubscriptionClicked(true); // Met à jour l'état pour indiquer que le bouton "S'abonner" a été cliqué
  }

  return (
    <div>
      <Link>besoin d'aide ? Contactez nous !</Link>
