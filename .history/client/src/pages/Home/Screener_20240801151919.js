...
export default function Screener() {
  ...
  const handleError = async (e, symbol) => {
    e.target.style.display = 'none'; // Masque l'image invalide
    const logoUrl = await getLogoUrl(symbol); // Obtient le logo depuis Financial Modeling Prep

    if (logoUrl === defaultLogoUrl) {
      // Si le logo depuis Financial Modeling Prep est également invalide
      ...
      // Sinon, affiche le logo de Financial Modeling Prep
      const newImage = document.createElement('img');
      newImage.src = logoUrl;
      newImage.alt = 'Logo Financial Modeling Prep';
      newImage.width = 50;
      newImage.height = 50;
      newImage.style.marginRight = '10px';
      newImage.onerror = () => { newImage.src = defaultLogoUrl; }; // Gestion des erreurs pour le nouveau logo
      e.target.parentNode.appendChild(newImage); // Ajoute le nouveau logo à la cellule
    }
  };
