...
export default function Screener() {
  ...
  const handleSortChange = (order) => {
    setSortOrder(order);
    let sortedInfos = [...filteredInfos];
    if (order === 'asc') {
      sortedInfos.sort((a, b) => a.mktCap - b.mktCap);
    } else if (order === 'desc') {
      sortedInfos.sort((a, b) => b.mktCap - a.mktCap);
    } else {
      sortedInfos = [...infos];  // Reset to the original fetched data order
    }
    setFilteredInfos(sortedInfos);
  };
        ...
        <div className="col-md-6 d-flex justify-content-end">
          ...
          <div className="dropdown">
            ...
            <div className="dropdown-menu" aria-labelledby="dropdownMenuButton">
              <button className="dropdown-item" onClick={() => handleSortChange('asc')}>Croissant</button>
              <button className="dropdown-item" onClick={() => handleSortChange('desc')}>Décroissant</button>
              <button className="dropdown-item" onClick={() => handleSortChange('default')}>Défaut</button>
            </div>
          </div>
        </div>
      </div>
