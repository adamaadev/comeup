...
const totalScore = netIncomeCount + operatingCashFlowCount + roaCount + cptnetoperating + cptcurrentratio + cptlongtermdebttotalasset + cptsharesoutstanding + cptmargebrute;
 ...
 useEffect(() => {
  ...
  const formule = (incomtax23 / capitaux23) * 100; 
  const roce23 = formule.toFixed(2)
  const pct1 = (((roce19 + roce20 + roce21 + roce22 + formule) / 5)).toFixed(2);
  ...
  const buyback = (-((commonstockrepurchased + commonstockissued) / pct) *100)
