  ...
  return (
    <div>
     {(isNaN(App) || isFinite(App) || App === null) ? '' : <p>Buyback yield 1 an {symbol} : {App} %</p>}      
      <p>{(isNaN(roce23) || !isFinite(roce23) || roce23 === null) ? '' : `Roce 1 an : ${roce23} %`}</p>
      <p>{(isNaN(pct1) || !isFinite(pct1) || pct1 === null) ? '' : `Roce 5 ans : ${pct1} %`}</p>
      <p>{cagr5Years !== null ? `${cagr5Years}%` : 'Calcul en cours...'}</p>
      <p>{(isNaN(payoutRatio) || !isFinite(payoutRatio) || payoutRatio === null) ? '' : `Payout Ratio de ${symbol} : ${payoutRatio}%`}</p>
      <p>{(isNaN(fcfMarginOneYear) || !isFinite(fcfMarginOneYear) || fcfMarginOneYear === null) ? '' : `Free Cash Flow Margin 1 an : ${fcfMarginOneYear}%`}</p>
      <p>{(isNaN(averageMarginFiveYears) || !isFinite(averageMarginFiveYears) || averageMarginFiveYears === null) ? '' : `Free Cash Flow Margin 5 ans : ${averageMarginFiveYears}%`}</p>
      <p>{(isNaN(annualizedGrowthRate) || !isFinite(annualizedGrowthRate) || annualizedGrowthRate === null) ? '' : `Croissance Annualisée du Dividende : ${annualizedGrowthRate}%`}</p>
      <p>{(isNaN(netIncomeGrowthOneYear) || !isFinite(netIncomeGrowthOneYear) || netIncomeGrowthOneYear === null) ? '' : `Croissance du résultat net sur 1 an: ${netIncomeGrowthOneYear}%`}</p>
      <p>{(isNaN(netIncomeGrowthFiveYears) || !isFinite(netIncomeGrowthFiveYears) || netIncomeGrowthFiveYears === null) ? '' : `Croissance du résultat net sur 5 ans: ${netIncomeGrowthFiveYears}%`}</p>
      <p>{(isNaN(netIncomeGrowthTenYears) || !isFinite(netIncomeGrowthTenYears) || netIncomeGrowthTenYears === null) ? '' : `Croissance du résultat net sur 10 ans: ${netIncomeGrowthTenYears}%`}</p>
      <p>{(isNaN(averageRatio) || !isFinite(averageRatio) || averageRatio === null) ? '' : `Capex / Résultat net (moyenne 5 ans) : ${averageRatio}%`}</p>
      <p>{(isNaN(debtToEquityRatio) || !isFinite(debtToEquityRatio) || debtToEquityRatio === null) ? '' : `Debt to Equity Ratio de : ${debtToEquityRatio}`}</p>
      <p>{(isNaN(netSharesRepurchases) || !isFinite(netSharesRepurchases) || netSharesRepurchases === null) ? '' : `Rachat net moyen d'actions (moyenne 5 ans) ${symbol}: ${netSharesRepurchases}`}</p>
      <p>{(isNaN(growthRate) || !isFinite(growthRate) || growthRate === null) ? '' : `Croissance moyenne du dividende sur 5 ans ${symbol}: ${growthRate.toFixed(2)} %`}</p>
      <p>{(isNaN(currentStreak) || !isFinite(currentStreak) || currentStreak === null) ? '' : `Le dividende n'a pas diminué depuis ${currentStreak} an(s)`}</p>
      <p>{(isNaN(cagr1Year1) || !isFinite(cagr1Year1) || cagr1Year1 === null) ? '' : `CAGR sur 1 an : ${cagr1Year1}`}</p>
      <p>{(isNaN(cagr5Years) || !isFinite(cagr5Years) || cagr5Years === null) ? '' : `CAGR sur 5 ans : ${cagr5Years}`}</p>
      <p>{(isNaN(cagr10Years3) || !isFinite(cagr10Years3) || cagr10Years3 === null) ? '' : `CAGR sur 10 ans : ${cagr10Years3}`}</p>
      <p>{(isNaN(totalScore) || !isFinite(totalScore) || totalScore === null) ? '' : `Total Piotroski Score: ${totalScore} / 9 points`}</p>
