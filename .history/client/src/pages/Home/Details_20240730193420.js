...
export default function CompanyDetails() {
  ...
  // Function to parse and classify each ratio based on percentiles
  const parseValue = (value) => {
    const numericValue = parseFloat(value.replace(',', '.')); // Convert to float
    return isNaN(numericValue) ? null : numericValue;
  };

  const classifyRatio = (value, percentiles) => {
    const numericValue = parseValue(value);
    if (numericValue === null) return 'Données indisponibles';
