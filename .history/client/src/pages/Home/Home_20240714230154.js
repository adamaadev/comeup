...
export default function Home() {
  ...
  return (
    <div>
      <header className="fixed-top bg-light w-100 shadow-sm">
          ...
          <div className="collapse navbar-collapse" id="navbarNav">
            ...
            <form className="form-inline my-2 my-lg-0 position-relative">
              <input 
                ...
                type="search" 
                placeholder="Entrez le symbole ou nom" 
                value={state} 
                onChange={(e) => setState(e.target.value)} 
                aria-label="Search"
              />
              {suggestions.length > 0 && (
                <ul className="list-group position-absolute w-100" style={{ zIndex: 1000, top: '40px' }}>
