...
export default function Foire({ symbol }) {
  ...
  const handleModalOpen = async (question) => {
    setModalQuestion(question);

    try {
      const response = await axios.post('http://localhost:4000/getResponseForQuestion', { id, symbol, question });
      const { response: selectedResponse, justification } = response.data;
      setSelectedResponse(selectedResponse);
      setJustification(justification);
      setExistingResponses({
        ...existingResponses,
        [question]: { response: selectedResponse, justification }
      });
    } catch (error) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await axios.post('http://localhost:4000/updateResponse', { id, symbol, modalQuestion, selectedResponse, justification });
      setSelectedResponse("");
      setJustification("");
      setModalQuestion(null);
      const scoreResponse = await axios.post('http://localhost:4000/checkscore', { id, symbol });
  ...
  return (
    <div>
      <div className="mt-4">
        ...
        {/* Autres catégories de questions (positionnement, management, autres) à ajouter de la même manière */}
