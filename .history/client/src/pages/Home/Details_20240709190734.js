          ...
          <div>
            ...
            <ul>
              {forces.map((forceItem, index) => (
                <li key={index}>{forceItem.content} <button className='btn btn-danger' onClick={(e) => { e.preventDefault(); supprimer(forceItem.id) }}>supprimer</button></li>
              ))}
            </ul>
            ...
            <ul>
              {risques.map((risqueItem, index) => (
                <li key={index}>{risqueItem.content} <button className='btn btn-danger' onClick={(e) => { e.preventDefault(); supprimer(risqueItem.id) }}>supprimer</button></li>
              ))}
            </ul>
          </div>
        );
    ...
    <div className="container mt-4">
      <div className="card mb-3">
        <div className="card-header">
          ...
          {exist ? (
            <button className="btn btn-danger btn-sm float-right" onClick={() => deleteItem(symbol)}>Retirer de la watchlist</button>
          ) : (
            <button className="btn btn-success btn-sm float-right" onClick={() => add(symbol)}>Ajouter à la watchlist</button>
          )}
        </div>
        <div className="tabs">
          <button className={`tab-button ${activeTab === 'profile' ? 'active' : ''}`} onClick={() => setActiveTab('profile')}>Informations</button>
          <button className={`tab-button ${activeTab === 'news' ? 'active' : ''}`} onClick={() => setActiveTab('news')}>Analyse</button>
          <button className={`tab-button ${activeTab === 'financials' ? 'active' : ''}`} onClick={() => setActiveTab('financials')}>Statistiques</button>
        </div>
