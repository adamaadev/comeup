  ...
  useEffect(() => {
    axios.get(`https://financialmodelingprep.com/api/v3/profile/${symbol}?apikey=7DpR3qkhqfZy2qfQDZZwHrz0Ohd6oRUX`)
      .then(res => {
        ...
        setInfos([res.data[0]]) 
        const {name , image , symbol , country , marketcap , secteur} = res.data[0]
        console.log(res.data[0].companyName , res.data[0].image , res.data[0].symbol , res.data[0].country , res.data[0].mktCap , res.data[0].sector);
      });
  }, [symbol]);
