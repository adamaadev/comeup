...
export default function Screener() {
  ...
  const fetchRandomCompanies = () => {
    axios.get(`http://localhost:4000/listcompany?page=${page}&limit=1000`)
      .then(res => {
        ...
        <table className="table table-striped table-hover">
          ...
          <tbody>
            {filteredInfos.map((company, index) => (
              <tr key={company.symbol} onClick={(e) => {
                e.preventDefault();
                const newTab = window.open(`/details/${company.symbol}`, '_blank');
                setTimeout(() => {
                  if (newTab) {
                    newTab.blur(); // Remove focus from the new tab
                    window.focus(); // Return focus to the current window
                  }
