...
export default function App({symbol}) {
  ...
  const [chiffreasetchiffreaffaire22, setchiffreaffaire22] = useState(null);
    ...
    const fetchFinancialData = async () => {
      try {
        ...
        if (cashFlowDataOneYear.length > 0 && incomeDataOneYear.length > 0) {
          ...
          const chiffreaffaire = (fcfOneYear / revenueOneYear) * 100;
          setchiffreaffaire22(chiffreaffaire);
        } else {
          setchiffreaffaire22('Données de cash-flow ou de revenu insuffisantes');
        }
...
{chiffreasetchiffreaffaire22 !== null && (
  <p>Free Cash Flow Margin 1 an : {chiffreasetchiffreaffaire22}%</p>
)}
