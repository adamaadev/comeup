  ...
  useEffect(() => {
    axios.get(`https://financialmodelingprep.com/api/v3/profile/${symbol}?apikey=7DpR3qkhqfZy2qfQDZZwHrz0Ohd6oRUX`)
      .then(res => {
        ...
        // Vérifier si l'image est undefined ou 'error'
        if (!image || image === 'error') {
          // Faire une requête vers Clearbit pour obtenir un nouveau logo
          axios.get(`https://logo.clearbit.com/${companyName.toLowerCase().replace(/\s/g, '')}.com`)
            ...
            .catch(error => {
              console.error('Erreur lors de la requête vers Clearbit:', error);
              setInfos([res.data[0]]); // Garder les données d'origine en cas d'erreur
            });
        } else {
          setInfos([res.data[0]]);
        }
  
        // Envoyer les informations à votre serveur local
        axios.post('http://localhost:4000/checkcompany', { companyName, image, symbol, country, mktCap, sector })
          .catch(error => {
            console.error('Erreur lors de la requête POST vers localhost:', error);
          });
      })
      .catch(error => {
        console.error('Erreur lors de la requête GET vers FinancialModelingPrep:', error);
      });
  }, [symbol]);
