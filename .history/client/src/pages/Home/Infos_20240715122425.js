 ...
 useEffect(() => {
  ...
  return (
    <div>   
      ...
      {isNaN(buyback) ? '' : <p>Roce 1 an : {roce23} %</p>}
      {isNaN(pct1) ? '' : <p>Roce 5 ans : {pct1} %</p>}
      <p>Performance moyenne sur les 5 dernières années pour {symbol}  : {cagr5Years !== null ? `${cagr5Years.toFixed(2)}%` : 'Calcul en cours...'}</p>
      ...
      <p>FCF 1 an : {cagr1Year1 !== null ? `${cagr1Year1}%` : 'Calcul en cours...'}</p>
      <p>FCF sur 5 ans : {cagr5Years2 !== null ? `${cagr5Years2}%` : 'Calcul en cours...'}</p>
      <p>FCF sur 10 ans : {cagr10Years3 !== null ? `${cagr10Years3}%` : 'Calcul en cours...'}</p>
      <p>Croissance CA 1 an ({dateOneYearAgo} à {dateLastYear}) : {croissanceCA1an.toFixed(2)}%</p>
      <p>Croissance CA 5 ans ({dateFiveYearsAgo} à {dateLastYear}) : {croissanceCA5ans.toFixed(2)}%</p>
      <p>Croissance CA 10 ans ({dateTenYearsAgo} à {dateLastYear}) : {croissanceCA10ans.toFixed(2)}%</p>
    </div>
  )
}