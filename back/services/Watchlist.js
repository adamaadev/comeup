...
export function WatchlistUser(req, res) {
  ...
  con.query("SELECT symbol FROM watchlist WHERE id_user = ? AND type_user = 'user'", [id], (err, result) => {
    if (err) {
      return res.status(500).send(err);
    }
...
export function AddCompanyadmin (req,res) {
  const { symbol, type } = req.body;
  console.log(symbol , type);
  
  con.query('INSERT INTO watchlist VALUES (?, ?, ?,?)',[null , "0", symbol , type],(err,result)=>{
    if(!err){
      res.send({success : true})
    }
  })
...
export function deletecompanyadmin(req, res) {
  const { symbol, type } = req.body;
  con.query("DELETE FROM watchlist WHERE symbol = ? AND type_user = ?", [symbol, type], (err, result) => {
    if (err) {
      res.status(500).send({ success: false, message: 'Erreur lors de la suppression' });
...
export function checkcompanyadmin(req, res) {
const { symbol, type } = req.body;
con.query("SELECT * FROM watchlist WHERE symbol = ? AND type_user = ?", [symbol, type], (err, result) => {
  if (err) {
    console.error(err);
    return res.status(500).send('Server Error');
  }
