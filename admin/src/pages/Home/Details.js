...
export default function Details() {
  ...
  useEffect(() => {
      axios.post('http://localhost:4000/checkcompanyadmin', { symbol,  type })
        .then(res => setExist(res.data.exist));
  }, [symbol]);

  const add = () => {
    axios.post('http://localhost:4000/addcompanyadmin', { symbol,  type })
      .then(res => {
        if (res.data.success) {
          setExist(true);
        }
  ...
  const removeFromWatchlist = () => {
    axios.post('http://localhost:4000/deletecompanyadmin', { symbol,  type })
      .then(res => {
        if (res.data.success) {
          setExist(false);
        }
